...
public sealed class MapValidator : IMapValidator
{
    ...
    private static async Task<SchemaValidationContext?> BuildSchemaValidationContextAsync(CancellationToken ct)
    {
        ...
        var schemaAssembly = typeof(SchemaIds).Assembly;
        var schemaResourceNames = schemaAssembly.GetManifestResourceNames()
            .Where(n => n.Contains("Resources.Schemas") && n.EndsWith(".schema.json"))
            .ToList();
        ...
        foreach (var resourceName in schemaResourceNames)
        {
            ...
            try
            {
                await using var stream = schemaAssembly.GetManifestResourceStream(resourceName);
