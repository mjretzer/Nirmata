...
public sealed class SymbolCacheBuilder : ISymbolCacheBuilder
{
    ...
    private sealed class SymbolCache
    {
        ...
        public SymbolCache(string? cacheDir, bool enabled)
        {
            ...
            _cacheDir = cacheDir ?? Path.Combine(Path.GetTempPath(), "gmsd-symbol-cache");
            if (_enabled)
            {
    ...
    private static readonly Regex TypePattern = new(
        @"^(\s*)(public|private|internal|protected|file)?\s*(static|abstract|sealed|partial)?\s*(class|interface|struct|enum|record)\s+(\w+)",
        RegexOptions.Compiled);

    private static readonly Regex MethodPattern = new(
        @"^(\s*)(public|private|internal|protected)?\s*(static|abstract|virtual|override|sealed|async)?\s*(\S+)\s+(\w+)\s*\(",
        RegexOptions.Compiled);

    private static readonly Regex PropertyPattern = new(
        @"^(\s*)(public|private|internal|protected)?\s*(static|abstract|virtual|override|sealed)?\s*(\S+)\s+(\w+)\s*[{=]",
        RegexOptions.Compiled);

    private static readonly Regex FieldPattern = new(
        @"^(\s*)(public|private|internal|protected|readonly)?\s*(static|readonly|const)?\s*(\S+)\s+(\w+)\s*[;=]",
        RegexOptions.Compiled);
        ...
        string[] lines, int startLine, string filePath, string relativePath)
    {
        ...
        var nextLine = isBlock ? FindClosingBrace(lines, startLine) : startLine + 1;
        ...
        var symbol = new SymbolInfo
        {
            ...
            Namespace = "",
            Location = CreateLocation(filePath, relativePath, startLine, line, name)
        };

        return (symbol, nextLine);
    }
        ...
        Match match, SymbolCacheOptions options)
    {
        ...
        if (!ShouldIncludeSymbol(accessibility, options))
            return (null, FindClosingBrace(lines, startLine) + 1);
    ...
    private static int FindClosingBrace(string[] lines, int startLine)
    {
        ...
        for (var i = startLine; i < lines.Length; i++)
        {
            ...
            if (depth == 0 && (line.Contains("}") || i == startLine))
                return i;
        }
