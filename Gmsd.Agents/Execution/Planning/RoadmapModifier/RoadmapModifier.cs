...
public sealed class RoadmapModifier : IRoadmapModifier
{
    ...
    private async Task<bool> IsPhaseActiveAsync(string phaseId, CancellationToken ct)
    {
        try
        {
            ...
            var statePath = Path.Combine(_specStore.Inner.GetType()
                .GetField("_aosRootPath", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.GetValue(_specStore.Inner)?.ToString() ?? ".aos", ".aos/state/state.json");
    ...
    private async Task<string> CreateBlockerIssueAsync(string phaseId, string runId, CancellationToken ct)
    {
        ...
        var issueDoc = new
        {
            schema = "gmsd:aos:schema:issue:v1",
            issueId = issueId,
