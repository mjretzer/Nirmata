...
public sealed class VerifierHandler
{
    ...
    public async Task<CommandRouteResult> HandleAsync(CommandRequest request, string runId, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var (criteria, fileScopes) = await ExtractAcceptanceCriteriaAsync(planPath, ct);
        ...
        {
            return CommandRouteResult.Failure(99, $"Verifier handler failed: {ex.Message}");
        }
    }
    ...
    private static async Task<(IReadOnlyList<AcceptanceCriterion> Criteria, IReadOnlyList<FileScope> FileScopes)> ExtractAcceptanceCriteriaAsync(string planPath, CancellationToken ct)
    {
        ...
        try
        {
                ...
                fileScopesElement.ValueKind == JsonValueKind.Array)
            {
                foreach (var scopeElement in fileScopesElement.EnumerateArray())
                {
                    ...
                    {
                        var relativePath = scopeElement.GetProperty("relativePath").GetString() ?? "";
