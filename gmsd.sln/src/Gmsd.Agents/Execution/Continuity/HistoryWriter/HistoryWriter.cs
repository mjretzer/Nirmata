...
internal sealed class HistoryWriter : IHistoryWriter
{
    ...
    public bool Exists(string runId, string? taskId = null)
    {
        ...
        try
        {
            ...
            // Use Regex to ensure we match the exact key as a header, preventing partial matches
            // e.g., "### runId" should not match "### runId/taskId"
            return Regex.IsMatch(content, $@"^### {Regex.Escape(key)}\s*$", RegexOptions.Multiline);
        }
        catch
        {
            return false;
