...
This guide explains how to add support for new LLM providers to the GMSD Agents system using Semantic Kernel.

## Architecture Overview

The system uses a **provider selection pattern** based on configuration:

1. **Configuration-driven selection**: The active provider is determined by `GmsdAgents:SemanticKernel:Provider` setting
...
Create a new configuration class in `Gmsd.Agents/Configuration/`:
...
Update `Gmsd.Agents/Configuration/SemanticKernelOptions.cs`:
...
1. **Unit tests**: Create tests in `tests/Gmsd.Agents.Tests/Configuration/`
   - Test configuration validation
   - Test DI registration
   - Test error handling for missing/invalid config

2. **Integration tests**: Create tests in `tests/Gmsd.Agents.Tests/Execution/ControlPlane/Llm/`
