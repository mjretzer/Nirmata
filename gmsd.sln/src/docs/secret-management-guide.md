...
The GMSD orchestrator uses secure secret management to protect sensitive credentials like API keys. Secrets are stored in the Windows Credential Manager (or equivalent OS keychain on other platforms) rather than in plaintext configuration files.
...
3. Check that the GMSD service account has credential store permissions
