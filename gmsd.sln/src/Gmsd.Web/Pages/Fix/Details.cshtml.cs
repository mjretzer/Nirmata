...
public class DetailsModel : PageModel
{
    ...
    public DiagnosticArtifactViewModel? FixPlanDiagnostic { get; set; }
    public ArtifactValidationStatusViewModel? FixPlanValidationStatus { get; set; }
    ...
    private void LoadFixPlanTasks(string loopId)
    {
        ...
        try
        {
            if (System.IO.File.Exists(tasksPath))
            {
                ...
                // Load fix plan diagnostic
                LoadDiagnosticForArtifact(tasksPath, "fix", loopId, out var fixPlanDiag, out var fixPlanStatus);
                FixPlanDiagnostic = fixPlanDiag;
                FixPlanValidationStatus = fixPlanStatus;
            }
        }
        catch (Exception ex)
        {
