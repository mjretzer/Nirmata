...
public class DetailsModel : PageModel
{
    ...
    public DiagnosticArtifactViewModel? PlanDiagnostic { get; set; }
    public DiagnosticArtifactViewModel? UatDiagnostic { get; set; }
    public ArtifactValidationStatusViewModel? PlanValidationStatus { get; set; }
    ...
    private void LoadTaskArtifacts(string taskId)
    {
        ...
        if (System.IO.File.Exists(planFile))
        {
            try
            {
                ...
                LoadDiagnosticForArtifact(planFile, "spec/tasks", taskId, out var planDiag, out var planStatus);
                PlanDiagnostic = planDiag;
                PlanValidationStatus = planStatus;
            }
            catch (Exception ex)
            {
