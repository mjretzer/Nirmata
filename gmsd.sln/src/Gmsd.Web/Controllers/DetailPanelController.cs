...
public class DetailPanelController : Controller
{
    ...
    /// <summary>
    /// Deep link endpoint - loads content via URL query parameters
    /// Supports: ?page=/Projects, ?entityType=project&entityId=xyz
    /// </summary>
    [HttpGet("deeplink")]
        ...
        [FromQuery] string? entityId = null)
    {
        try
        {
            // Entity reference takes precedence
            ...
            if (!string.IsNullOrEmpty(page))
            {
                ...
                if (Request.Headers.ContainsKey("HX-Request"))
                {
                    // Load the page content as a partial
                    var viewName = GetViewNameForPage(pagePath);
                ...
                return Ok(new
                {
                    type = "page",
                    page = pagePath,
                    title = GetTitleForPage(pagePath)
                });
            }

            return BadRequest(new { error = "Missing required parameters: provide 'page' or both 'entityType' and 'entityId'" });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing deep link");
            return StatusCode(500, new { error = "Failed to process deep link" });
        }
    }

    /// <summary>
    /// Maps a page path to its corresponding view name
    /// </summary>
    private static string GetViewNameForPage(string pagePath)
    {
        var normalized = pagePath.ToLowerInvariant().Trim('/');
        return normalized switch
        {
            "" or "dashboard" => "_DashboardOverview",
            "projects" => "Projects/_ProjectList",
            "runs" => "Runs/_RunList",
            "tasks" => "Tasks/_TaskList",
            "specs" => "Specs/_SpecList",
            _ => "_EmptyState"
        };
    }

    /// <summary>
    /// Gets a title for a page path
    /// </summary>
    private static string GetTitleForPage(string pagePath)
    {
        var normalized = pagePath.ToLowerInvariant().Trim('/');
        return normalized switch
        {
