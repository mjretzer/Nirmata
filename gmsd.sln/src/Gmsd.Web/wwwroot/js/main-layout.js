...
(function() {
    ...
    class MainLayoutController {
        ...
        setupKeyboardShortcuts() {
            // Track keyboard shortcut help visibility
            this.helpOverlay = null;
            
            document.addEventListener('keydown', (e) => {
                // Don't trigger shortcuts when typing in inputs (except specific cases)
                const isInput = e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA';
                const isListNavigation = e.target.closest('[data-keyboard-nav]') !== null;
                ...
                // Cmd+? or Shift+/ to show keyboard shortcuts help
                if ((e.metaKey && e.key === '?') || (e.key === '?' && e.shiftKey && !isInput)) {
                    e.preventDefault();
                    this.toggleKeyboardShortcutsHelp();
                    return;
                }

                // Escape to collapse panels, clear selection, close overlays
                if (e.key === 'Escape') {
                    this.handleEscapeKey();
                    return;
                }

                // Cmd+[ to navigate back in history
                if (e.metaKey && e.key === '[') {
                    e.preventDefault();
                ...
                // Cmd+] to navigate forward in history
                if (e.metaKey && e.key === ']') {
                    e.preventDefault();
                ...
                // List navigation with arrow keys (when focused on a list container)
                if (isListNavigation && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
                    e.preventDefault();
                    this.navigateList(e.target.closest('[data-keyboard-nav]'), e.key);
                    return;
                }
            });
        }
        ...
        handleEscapeKey() {
            ...
            // Close any open detail panel selection
            const detailPanelContent = document.querySelector('#detail-panel .panel-content');
            if (detailPanelContent && detailPanelContent.dataset.hasSelection) {
                delete detailPanelContent.dataset.hasSelection;
                detailPanelContent.querySelectorAll('.selected').forEach(el => el.classList.remove('selected'));
            }

            // Collapse panels if neither is collapsed
            if (!this.state.sidebar.collapsed || !this.state.detail.collapsed) {
                this.sidebar.classList.add('collapsed');
                this.detailPanel.classList.add('collapsed');
                this.state.sidebar.collapsed = true;
                this.state.detail.collapsed = true;
            ...
            // Blur and clear chat input as last resort
        ...
        navigateList(listContainer, key) {
            const items = listContainer.querySelectorAll('[data-list-item], li, .list-item');
        ...
        showKeyboardShortcutsHelp() {
            const shortcuts = [
                { keys: ['/ or Cmd+K'], description: 'Focus chat input' },
                { keys: ['Esc'], description: 'Collapse panels, clear selection' },
                { keys: ['Cmd+['], description: 'Navigate back' },
                { keys: ['Cmd+]'], description: 'Navigate forward' },
                { keys: ['Cmd+1'], description: 'Toggle sidebar' },
                { keys: ['Cmd+2'], description: 'Toggle detail panel' },
                { keys: ['Cmd+3'], description: 'Toggle both panels' },
                { keys: ['Cmd+?'], description: 'Show/hide keyboard shortcuts' },
                { keys: ['↑ ↓'], description: 'Navigate lists (when focused)' },
                { keys: ['Enter'], description: 'Select list item' },
            ...
            const html = `
                <div class="keyboard-shortcuts-overlay" id="keyboard-shortcuts-help">
                        ...
                        <div class="shortcuts-list">
                            ${shortcuts.map(s => `
                                <div class="shortcut-item">
                                    <kbd class="shortcut-keys">${s.keys.map(k => k.replace('Cmd+', cmdKey)).join(' ')}</kbd>
            ...
            this.helpOverlay = document.getElementById('keyboard-shortcuts-help');
            ...
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && this.helpOverlay) {
                    this.closeKeyboardShortcutsHelp();
                }
            }, { once: true });
        }
