...
Defines the current implemented UAT UI in `Gmsd.Web` for performing per-task verification sessions and recording pass/fail results back to workspace artifacts.

- **Lives in:** `Gmsd.Web/Pages/Uat/*`, `.aos/spec/uat/*.json`, `.aos/spec/tasks/<task-id>/uat.json`
