# engine-schema-registry-service Specification

## Purpose

Defines the public DI/interface surface for retrieving JSON Schemas by `$id` in the AOS engine. Canonical schema pack and registry semantics are defined by `aos-schema-registry`, and this capability MUST conform.

- **Lives in:** `Gmsd.Aos/Public/ISchemaRegistry.cs`, `Gmsd.Aos/Public/Composition/*`, `.aos/schemas/**`
- **Owns:** Public interface shape and DI registration for schema retrieval and enumeration
- **Does not own:** Schema version policy or validation-gate behavior (owned by canonical schema registry spec)
## Requirements
### Requirement: Schema registry interface exists
The system SHALL define `ISchemaRegistry` as a public interface in `Gmsd.Aos/Public/`.

The interface SHALL provide methods to load and retrieve JSON schemas by their `$id`.

#### Scenario: Load schema by $id
- **GIVEN** a valid schema `$id` (e.g., `gmsd:aos:schema:project:v1`)
...
The interface SHALL provide access to schemas embedded in the `Gmsd.Aos` assembly.

#### Scenario: Load embedded project schema
- **GIVEN** the embedded project schema with `$id` `gmsd:aos:schema:project:v1`
- **WHEN** `ISchemaRegistry.GetSchema("gmsd:aos:schema:project:v1")` is called
...
The system SHALL register `ISchemaRegistry` as a Singleton in `AddGmsdAos()`.
