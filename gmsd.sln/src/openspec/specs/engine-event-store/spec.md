...
- **Lives in:** `Gmsd.Aos/Public/IEventStore.cs`, `Gmsd.Aos/Public/Composition/*`, `.aos/state/events.ndjson`
...
The system SHALL define `IEventStore` as a public interface in `Gmsd.Aos/Public/`.
...
The system SHALL register `IEventStore` as a Singleton in `AddGmsdAos()`.
