...
Defines the current implemented Issues UI in `Gmsd.Web` for browsing issue artifacts from the selected workspace’s `.aos/spec/issues/**` directory and routing issues into fix planning.

- **Lives in:** `Gmsd.Web/Pages/Issues/*`, `.aos/spec/issues/*.json`
