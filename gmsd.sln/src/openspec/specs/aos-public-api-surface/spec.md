...
The system SHALL provide public service interfaces under `Gmsd.Aos/Public/Services/**` (`Gmsd.Aos.Public.Services.*`) for the engineâ€™s primary subsystems:
- Workspace
- Spec store
- State store
- Evidence store
...
The system SHALL provide a public state store contract `Gmsd.Aos.Public.IStateStore` that allows consumers to interact with the state layer (`.aos/state/*`) without referencing internal engine types.
...
Public state store APIs MUST exchange only `Gmsd.Aos.Contracts.State.*` types (or primitive CLR types) and MUST NOT expose types in `Gmsd.Aos.Engine.*` or `Gmsd.Aos._Shared.*`.

#### Scenario: Consumer reads snapshot using public state store
- **GIVEN** a consumer that references only `Gmsd.Aos.Public.*` and `Gmsd.Aos.Contracts.*`
- **WHEN** it reads the state snapshot via `IStateStore`
- **THEN** it receives a stable snapshot contract type without referencing `Gmsd.Aos.Engine.*` namespaces
...
The system SHALL provide stable contract DTOs under `Gmsd.Aos/Contracts/State/**` (`Gmsd.Aos.Contracts.State.*`) for the public state store surface.
...
- **THEN** it can do so using only `Gmsd.Aos.Contracts.State.*` and `Gmsd.Aos.Public.*` without referencing internal namespaces

### Requirement: DI registration extension method

The system SHALL provide an extension method `AddGmsdAos(this IServiceCollection services)` in `Gmsd.Aos/Composition/ServiceCollectionExtensions.cs` that registers all public AOS services with the .NET dependency injection container.
...
- **GIVEN** a consumer project (e.g., `Gmsd.Agents`) references `Gmsd.Aos`
- **WHEN** the consumer calls `services.AddGmsdAos()` in their composition root
...
- **WHEN** `services.AddGmsdAos(configuration)` is called
...
The system SHALL ensure the following public interfaces can be resolved from the DI container after calling `AddGmsdAos()`:

- `ICommandRouter` from `Gmsd.Aos.Public.Services`
- `IWorkspace` from `Gmsd.Aos.Public`
- `ISpecStore` from `Gmsd.Aos.Public`
- `IStateStore` from `Gmsd.Aos.Public`
- `IEvidenceStore` from `Gmsd.Aos.Public`
- `IValidator` from `Gmsd.Aos.Public`

#### Scenario: Gmsd.Agents resolves engine services
- **GIVEN** `Gmsd.Agents` calls `services.AddGmsdAos()`
...
The system SHALL provide an `AosOptions` class in `Gmsd.Aos/Configuration/` for engine configuration binding.
...
- **WHEN** `AddGmsdAos(configuration)` is called
