...
- Product Application projects MUST reference only lower layers in the product stack order (`Gmsd.Data.Dto` â†’ `Gmsd.Data` â†’ `Gmsd.Services` â†’ `Gmsd.Api` â†’ `Gmsd.Web`).
