...
Defines the current implemented Tasks UI in `Gmsd.Web` for browsing task specs and inspecting task artifacts from the selected workspace’s `.aos/spec/tasks/**` directory.

- **Lives in:** `Gmsd.Web/Pages/Tasks/*`, `.aos/spec/tasks/<task-id>/*`
- **Owns:** UI routing, filtering, and read-only rendering of task spec artifacts
- **Does not own:** Task planning/execution semantics (owned by engine/agents) or evidence generation contracts
## Requirements
### Requirement: Tasks List Page
The `Gmsd.Web` project SHALL provide a `/Tasks` page that displays all tasks with filtering capabilities.

The implementation MUST:
- Read tasks by enumerating `.aos/spec/tasks/<task-id>/task.json` under the selected workspace
...
- Link each task to its detail page (`/Tasks/Details/{taskId}`)
...
The `Gmsd.Web` project SHALL provide a `/Tasks/Details` page with tabbed interface showing all task spec artifacts.

The implementation MUST:
- Accept task ID via route parameter (`/Tasks/Details/{taskId}`)
...
- **WHEN** a user navigates to `/Tasks/Details/TSK-000001`
...
- **THEN** they navigate to `/Runs/Details/RUN-000001`
