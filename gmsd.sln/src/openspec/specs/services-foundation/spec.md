...
The system SHALL provide a centralized extension method `AddGmsdServices()` to register all service dependencies in the DI container.

#### Scenario: API composition
- **WHEN** the API application starts
- **THEN** it calls `builder.Services.AddGmsdServices()` to register all service interfaces and implementations
