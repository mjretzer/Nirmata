...
This change transforms the GMSD streaming model from a **synchronous job-summary pattern** to an **asynchronous agent-dialogue pattern**. The orchestrator becomes an observable event source that emits typed events as it reasons, decides, and responds.
...
- Or: Header-based negotiation `Accept: application/vnd.gmsd.streaming.v2+json`
