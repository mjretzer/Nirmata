...
- [x] 2.4 Expand `Gmsd.Aos.Public.IWorkspace` to include root paths and safe canonical path resolution helpers.

## 3. Verification
- [x] 3.1 Add deterministic path routing tests (IDs + non-ID contract paths).
- [x] 3.2 Add repo-root discovery tests: `.git` marker, `Gmsd.slnx` marker, and failure outside a repo.
