...
The current `Gmsd.Agents` layer implements a custom LLM abstraction (`ILlmProvider`, `LlmMessage`, etc.) with manual adapters for each provider (OpenAI, Anthropic, Azure, Ollama). This abstraction requires ongoing maintenance and lacks features like automatic function calling, retry policies, and streaming.
...
1. Add Semantic Kernel packages to `Gmsd.Agents.csproj`
