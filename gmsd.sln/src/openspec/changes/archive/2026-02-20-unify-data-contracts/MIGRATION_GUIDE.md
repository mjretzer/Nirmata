...
GMSD is upgrading to unified data contracts across all workflow phases. This ensures reliable artifact chaining between Phase Planning, Task Execution, Verification, and Fix Planning.
...
- Custom scripts that read or write GMSD artifacts
- Automated workflows that depend on artifact formats

**No, if you:**
- Are starting a new workspace
- Have only used the latest GMSD version
...
gmsd migrate-schemas --workspace-path C:\path\to\workspace
```
...
```bash
gmsd migrate-schemas --workspace-path C:\path\to\workspace --dry-run
```
...
3. **Validate artifacts**: Use the validation CLI command:
   ```bash
   gmsd validate-artifact --path .aos/spec/tasks/TSK-0001/plan.json
   ```
...
# Restore from backup
gmsd restore-backup --backup-path C:\path\to\workspace\..\backup-20260219-180700
```
...
If you have custom scripts that read or write GMSD artifacts:
...
**A:** The migration tool only handles standard GMSD artifact types. For custom artifacts:
...
1. Run `gmsd migrate-schemas --workspace-path <path> --dry-run` to preview
2. Run `gmsd migrate-schemas --workspace-path <path>` to migrate
