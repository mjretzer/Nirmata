...
This document provides a comprehensive guide for expanding LLM provider support in GMSD using Semantic Kernel.

## Overview

The GMSD Agents system uses a **configuration-driven provider selection pattern** that allows switching between different LLM providers without code changes. This guide explains the architecture and provides step-by-step instructions for adding new providers.
...
```
appsettings.json (GmsdAgents:SemanticKernel:Provider)
        ↓
...
1. **SemanticKernelOptions** (`Gmsd.Agents/Configuration/SemanticKernelOptions.cs`)
...
2. **SemanticKernelServiceCollectionExtensions** (`Gmsd.Agents/Configuration/SemanticKernelServiceCollectionExtensions.cs`)
...
3. **SemanticKernelLlmProvider** (`Gmsd.Agents/Execution/ControlPlane/Llm/Adapters/SemanticKernelLlmProvider.cs`)
...
- **Configuration**: `GmsdAgents:SemanticKernel:Provider = "OpenAi"`
...
- **Configuration**: `GmsdAgents:SemanticKernel:Provider = "AzureOpenAi"`
...
- **Configuration**: `GmsdAgents:SemanticKernel:Provider = "Anthropic"`
- **Required fields**: ApiKey, ModelId
- **Optional fields**: BaseUrl, ApiVersion (default: 2023-06-01)

### Ollama
- **Models**: Llama, Mistral, CodeLlama, etc.
- **Configuration**: `GmsdAgents:SemanticKernel:Provider = "Ollama"`
...
Create a new sealed class in `Gmsd.Agents/Configuration/SemanticKernelOptions.cs`:
    ...
    IConfiguration section)
{
    if (options is null)
    {
        throw new InvalidOperationException(
            ...
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider' section is configured with ApiKey and ModelId. " +
            "Example: {\"GmsdAgents\": {\"SemanticKernel\": {\"NewProvider\": {\"ApiKey\": \"...\", \"ModelId\": \"...\"}}}}");
    }
    ...
    if (string.IsNullOrWhiteSpace(options.ApiKey))
    {
        throw new InvalidOperationException(
            ...
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider:ApiKey' is set to your [NewProvider] API key. " +
    ...
    if (string.IsNullOrWhiteSpace(options.ModelId))
    {
        throw new InvalidOperationException(
            ...
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider:ModelId' is set to a valid model. " +
    ...
    IConfiguration configuration)
{
    ...
    if (options is null)
    {
        throw new InvalidOperationException(
            "[NewProvider] configuration is missing. " +
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider' section is configured.");
    }
    ...
    if (string.IsNullOrWhiteSpace(options.ApiKey))
    {
        throw new InvalidOperationException(
            ...
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider:ApiKey' is set.");
    }
    ...
    if (string.IsNullOrWhiteSpace(options.ModelId))
    {
        throw new InvalidOperationException(
            ...
            "Ensure 'GmsdAgents:SemanticKernel:NewProvider:ModelId' is set.");
    }
...
Create tests in `tests/Gmsd.Agents.Tests/Configuration/`:
...
public class NewProviderSemanticKernelOptionsTests
{
    ...
    public void ValidConfiguration_Loads_Successfully()
    {
        ...
        var config = new ConfigurationBuilder()
            ...
            {
                ["GmsdAgents:SemanticKernel:Provider"] = "NewProvider",
                ["GmsdAgents:SemanticKernel:NewProvider:ApiKey"] = "test-key",
                ["GmsdAgents:SemanticKernel:NewProvider:ModelId"] = "test-model"
            })
            .Build();

        // Act
        var options = config.GetSection("GmsdAgents:SemanticKernel").Get<SemanticKernelOptions>();
...
Create integration tests in `tests/Gmsd.Agents.Tests/Execution/ControlPlane/Llm/`:
...
public class NewProviderChatCompletionTests
{
    ...
    public async Task ChatCompletion_Returns_ValidResponse()
    {
        ...
        var config = new ConfigurationBuilder()
            ...
            {
                ["GmsdAgents:SemanticKernel:Provider"] = "NewProvider",
                ["GmsdAgents:SemanticKernel:NewProvider:ApiKey"] = Environment.GetEnvironmentVariable("NEWPROVIDER_API_KEY") ?? "test-key",
                ["GmsdAgents:SemanticKernel:NewProvider:ModelId"] = "test-model"
            })
            .Build();
