...
## What Changes
- Add a CLI command `aos init` (implemented in `Gmsd.Aos`) that creates a canonical `.aos/` skeleton (folders + baseline index files).
...
- **Affected code**: engine workspace contracts and CLI command handler in `Gmsd.Aos`, plus a fixture-based test that proves determinism across machines
