...
### Phase 1: Engine Enhancements (Gmsd.Aos) ✓
...
- `Gmsd.Aos/Engine/Workspace/AosWorkspaceBootstrapper.cs` - Added Repair method, RebuildIndexFiles, ValidateArtifactSchemas, ValidateWorkspaceLockIfPresent, ReadWorkspaceConfig, WriteWorkspaceConfig
- `Gmsd.Aos/Engine/Workspace/AosWorkspaceRepairResult.cs` - New file
- `Gmsd.Aos/Engine/Workspace/AosWorkspaceConfigDocument.cs` - New file

### Phase 2: Metadata & Repository (Gmsd.Data) ✓
...
**Files Created/Modified**:
- `Gmsd.Data/Entities/Workspaces/Workspace.cs` - Added LastValidatedAt property
- `Gmsd.Data/Repositories/IWorkspaceRepository.cs` - Added new methods
- `Gmsd.Data/Repositories/WorkspaceRepository.cs` - Implemented new methods
- `Gmsd.Data/Migrations/20260222201500_AddLastValidatedAtToWorkspace.cs` - New migration
- `Gmsd.Data/Migrations/20260222201500_AddLastValidatedAtToWorkspace.Designer.cs` - New designer file

### Phase 3: Web Services & API (Gmsd.Web) ✓
...
- **Config Migration Logic**: Implemented `MigrateWorkspaceConfigAsync()` that reads global config from `%LOCALAPPDATA%/Gmsd/workspace-config.json` and migrates workspace-specific settings to `.aos/config.json` using `AosWorkspaceBootstrapper.WriteWorkspaceConfig()`.
...
**Files Created/Modified**:
- `Gmsd.Web/Services/IWorkspaceService.cs` - Interface already existed, no changes needed
- `Gmsd.Web/Services/WorkspaceService.cs` - Added RepairWorkspaceAsync, enhanced ValidateWorkspaceAsync, added MigrateWorkspaceConfigAsync
- `Gmsd.Web/Controllers/WorkspaceController.cs` - Added structured error handling, input validation, logging
...
**Files Modified**:
- `Gmsd.Aos/Engine/Workspace/AosWorkspaceBootstrapper.cs` - Added repair duration tracking
- `Gmsd.Aos/Engine/Workspace/AosWorkspaceRepairResult.cs` - Added Duration property
- `Gmsd.Web/Services/WorkspaceService.cs` - Added comprehensive logging throughout
