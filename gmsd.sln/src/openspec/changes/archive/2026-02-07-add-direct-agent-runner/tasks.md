...
- [x] 1.1 Create `Gmsd.Web/Composition/ServiceCollectionExtensions.cs` with `AddGmsdAgents()` call
- [x] 1.2 Configure workspace path resolution for Web project (use temp path or configurable)
- [x] 1.3 Add required package references to `Gmsd.Agents` in `Gmsd.Web.csproj`
- [x] 1.4 Verify service starts without Windows Service dependency

## 2. Direct Agent Runner
- [x] 2.1 Create `Gmsd.Web/AgentRunner/WorkflowClassifier.cs` class
...
- [x] 3.1 Create `Gmsd.Web/Pages/Runs/Index.cshtml` with page model
...
## 4. Run Detail Page
- [x] 4.1 Create `Gmsd.Web/Pages/Runs/Details.cshtml` with page model
