...
| --- | --- | --- | --- |
| Task plan | `.aos/spec/tasks/<task-id>/plan.json` | `gmsd.task-plan` | `1` |
| Fix planning output (`plan.json`) | `.aos/spec/tasks/<task-id>/plan.json` (fix planning writer) | `gmsd.task-plan` | `1` |
| Command proposal artifact | runtime command-proposal payload/artifact boundary | `gmsd.command-proposal` | `1` |
...
```json
{
  ...
  "fileScopes": [
    {
      "path": "Gmsd.Agents/Execution/PlanWriter.cs"
    }
  ]
}
```
