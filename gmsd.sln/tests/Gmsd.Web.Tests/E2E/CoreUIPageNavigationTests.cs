...
public class CoreUIPageNavigationTests : IDisposable
{
    ...
    [InlineData("/Uat")]
    [InlineData("/Issues")]
    public async Task Get_Page_WithChatForwardUI_ReturnsSuccess(string url)
    {
        // Arrange
        var client = _factory.CreateClient();
        ...
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadAsStringAsync();
        content.Should().Contain("main-layout", $"Page {url} should use main-layout when feature flag is enabled");
    }
