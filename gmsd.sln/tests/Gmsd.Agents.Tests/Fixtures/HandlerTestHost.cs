...
public sealed class HandlerTestHost : IDisposable
{
    ...
    {
        // Bind configuration
        _services.Configure<Gmsd.Aos.Public.Configuration.AosOptions>(options => options.RepositoryRootPath = workspacePath);
        ...
        {
            var assembly = typeof(Gmsd.Agents.Configuration.ServiceCollectionExtensions).Assembly;
            return new EmbeddedResourcePromptLoader(assembly, "Gmsd.Agents.Prompts");
        });
