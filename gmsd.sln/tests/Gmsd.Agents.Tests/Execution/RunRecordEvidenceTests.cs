...
public class RunRecordEvidenceTests
{
    ...
    public RunRecordEvidenceTests()
    {
        ...
        // Setup default success response for tool calling loop
        _toolCallingLoopMock
            ...
            .ReturnsAsync(new ToolCallingResult
            {
                ...
                IterationCount = 1,
                Usage = new ToolCallingUsageStats { TotalPromptTokens = 10, TotalCompletionTokens = 10, IterationCount = 1 }
            });
    }
    ...
    public async Task RunSubagentAsync_ClosesRunRecord_WithFailureStatus_OnError()
    {
        ...
        // Simulate failure in tool calling loop
        _toolCallingLoopMock
            .Setup(x => x.ExecuteAsync(It.IsAny<ToolCallingRequest>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new Exception("Simulated tool execution failure"));
    ...
    public async Task RunSubagentAsync_RecordsToolCallEvidence_WhenToolCallsExist()
    {
        ...
        _toolCallingLoopMock
            ...
            .ReturnsAsync(new ToolCallingResult
            {
                ...
                ConversationHistory = new List<ToolCallingMessage>
                {
                    ToolCallingMessage.Assistant(toolCalls: new[]
                    {
                        new ToolCallingRequestItem { Id = "call-1", Name = "test-tool", ArgumentsJson = "{}" }
                    })
                },
                IterationCount = 1
            });
    ...
    public async Task RunSubagentAsync_IncludesErrorCategory_OnFailure()
    {
        ...
        {
            Console.WriteLine("Captured Data Keys: " + string.Join(", ", capturedData.Keys));
            if (capturedData.TryGetValue("error", out var err)) Console.WriteLine("Error: " + err);
        }
