...
using Gmsd.Agents.Execution.Migration;
using Gmsd.Aos.Public;
using Xunit;

namespace Gmsd.Agents.Tests.Execution.Migration;
...
public sealed class SchemaMigratorIntegrationTests : IAsyncLifetime
{
    ...
    {
        _testWorkspaceDir = Path.Combine(Path.GetTempPath(), $"gmsd-migration-test-{Guid.NewGuid()}");
    ...
    public async Task MigrateArtifactAsync_TransformsTaskPlanSuccessfully()
    {
        ...
        Assert.Equal("gmsd:aos:schema:task-plan:v1", schemaId.GetString());
    ...
    public async Task MigrateArtifactAsync_TransformsVerifierOutputSuccessfully()
    {
        ...
        Assert.True(root.TryGetProperty("schemaId", out var schemaId));
        Assert.Equal("gmsd:aos:schema:verifier-output:v1", schemaId.GetString());
    ...
    public async Task MigrateArtifactAsync_TransformsFixPlanSuccessfully()
    {
        ...
        Assert.True(root.TryGetProperty("schemaId", out var schemaId));
        Assert.Equal("gmsd:aos:schema:fix-plan:v1", schemaId.GetString());
    }
