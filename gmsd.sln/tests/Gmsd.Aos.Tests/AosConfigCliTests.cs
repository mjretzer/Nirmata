...
public sealed class AosConfigCliTests
{
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-config-validate-valid");
        try
        {
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-config-validate-missing");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-config-validate-plaintext");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
