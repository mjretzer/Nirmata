...
public sealed class AosValidateWorkspaceRequiredArtifactsTests
{
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-required-spec-index-missing");
        try
        {
    ...
    public async Task AosValidateWorkspace_FailsWhenRequiredSpecIndexIsMalformed(
        ...
        string expectedFailureLine)
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-required-spec-index-malformed");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-state-missing");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-state-malformed");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-events-missing");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-events-malformed");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-events-non-object");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-events-schema-violation");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
            ...
            Assert.Equal(2, exitCode);
            Assert.Contains(
                "FAIL [state] .aos/state/events.ndjson - (gmsd:aos:schema:event:v1 @ /lines/1/schemaVersion)",
                stdout,
                StringComparison.Ordinal
            );
            Assert.Contains("Validation failed:", stderr, StringComparison.Ordinal);
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-commands-missing");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-commands-malformed");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-run-manifest-missing");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-run-manifest-malformed");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
