...
- **Daemon traffic** (`VITE_DAEMON_URL`, default `http://localhost:9000`): health checks, service start/stop/restart, host-profile updates, AOS command execution — everything that controls or observes the engine host process. Always targets `nirmata.Windows.Service.Api`.
...
- Both default to `http://localhost:9000` in development, but are configured independently.
