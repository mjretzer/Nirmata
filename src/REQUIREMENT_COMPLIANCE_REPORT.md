...
The `InputClassifier` (`@/nirmata.Agents/Execution/Preflight/InputClassifier.cs:1-189`) implements explicit command prefix requirement:
...
The `ChatStreamingController.ExecuteCommandInternalV2` (`@/nirmata.Web/Controllers/ChatStreamingController.cs:268-312`) enforces this:
...
**Confirmation Gate:**
When write operations are detected, the system requires explicit user confirmation before execution (`@/nirmata.Agents/Execution/ControlPlane/Orchestrator.cs:239-310`).
...
The `LlmChatResponder` (`@/nirmata.Agents/Execution/ControlPlane/Chat/LlmChatResponder.cs:1-241`) provides full LLM integration:
...
**Integration Points:**
- `ChatResponder` adapter (`@/nirmata.Agents/Execution/ControlPlane/ChatResponder.cs:1-51`)
- `ResponderHandler` phase handler (`@/nirmata.Agents/Execution/ControlPlane/ResponderHandler.cs:1-43`)
- Registered in DI (`@/nirmata.Agents/Configuration/nirmataAgentsServiceCollectionExtensions.cs:185-207`)
...
The `StreamingOrchestrator` (`@/nirmata.Web/Models/Streaming/StreamingOrchestrator.cs:1-322`) emits structured events:
...
**GateSelectedPayload Structure** (`@/nirmata.Web/Models/Streaming/EventPayloads.cs:152-176`):
```csharp
...
**GatingEngine Provides Detailed Reasoning** (`@/nirmata.Agents/Execution/ControlPlane/GatingEngine.cs:33-150`):
```csharp
...
**Event Sink Extensions** (`@/nirmata.Web/Models/Streaming/EventSinkExtensions.cs:106-136`):
Provides fluent API for emitting gate.selected events with full context.

**Sequence Number Support:**
Events include monotonic sequence numbers for ordering (`@/tests/nirmata.Web.Tests/Models/Streaming/StreamingOrchestratorTests.cs:622-661`).
...
The `SemanticKernelLlmProvider` (`@/nirmata.Agents/Execution/ControlPlane/Llm/Adapters/SemanticKernelLlmProvider.cs:1-640`) provides complete end-to-end integration:
...
**Schema Definition System** (`@/nirmata.Agents/Execution/ControlPlane/Llm/Contracts/LlmStructuredOutputSchema.cs:1-96`):
```csharp
...
**Validation Enforcement** (`@/nirmata.Agents/Execution/ControlPlane/Llm/Adapters/SemanticKernelLlmProvider.cs:44-93`):
```csharp
...
**Usage in PhasePlanner** (`@/nirmata.Agents/Execution/Planning/PhasePlanner/PhasePlanner.cs:176-241`):
```csharp
...
**Usage in FixPlanner** (`@/nirmata.Agents/Execution/FixPlanner/FixPlanner.cs:710-770`):
```csharp
...
**Retry Logic** (`@/nirmata.Agents/Execution/ControlPlane/Llm/LlmRetryHandler.cs:1-127`):
- Exponential backoff for schema validation failures
- Up to 3 retries with enhanced system prompts
- Detailed error logging

**Command Suggestion Schema** (`@/nirmata.Agents/Execution/Preflight/CommandSuggestion/LlmCommandSuggester.cs:19-112`):
```csharp
var request = new LlmCompletionRequest
{
    Messages = [...],
...
**Diagnostic Artifacts** (`@/nirmata.Agents/Execution/FixPlanner/FixPlanner.cs:772-818`):
