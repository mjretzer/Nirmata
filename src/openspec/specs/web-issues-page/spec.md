...
Defines the current implemented Issues UI in `nirmata.Web` for browsing issue artifacts from the selected workspace’s `.aos/spec/issues/**` directory and routing issues into fix planning.

- **Lives in:** `nirmata.Web/Pages/Issues/*`, `.aos/spec/issues/*.json`
- **Owns:** UI routing, filtering, and read-only rendering of issue artifacts
- **Does not own:** Issue creation semantics (owned by verification workflows) or fix planning implementation details
## Requirements
### Requirement: Issues List Page
The `nirmata.Web` project SHALL provide an `/Issues` page that displays all issues with comprehensive filtering capabilities.

The implementation MUST:
- Read issues by enumerating `.aos/spec/issues/*.json` under the selected workspace
...
- Link each issue to its detail page (`/Issues/Details/{issueId}`)
...
The `nirmata.Web` project SHALL provide an `/Issues/Details` page that displays comprehensive information about a single issue.

The implementation MUST:
- Accept issue ID via route parameter (`/Issues/Details/{issueId}`)
...
- Link to associated phase (`/Phases/Details/{phaseId}`)
- Link to associated milestone (`/Milestones/Details/{milestoneId}`)
- Link to parent UAT session if applicable (`/Uat/Verify?taskId={taskId}&sessionId={sessionId}`)
...
- **WHEN** a user navigates to `/Issues/Details/ISS-0001`
