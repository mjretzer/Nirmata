...
Defines the current implemented Razor Pages shell for `nirmata.Web`, including layout/static assets and the baseline product-facing pages (Projects) plus engine-facing operational pages.

- **Lives in:** `nirmata.Web/Pages/**`, `nirmata.Web/wwwroot/**`
...
The nirmata.Web project SHALL provide Razor Pages infrastructure with proper view imports, layout templates, and static file support.

#### Scenario: View imports configured
- **WHEN** the application renders any Razor page
- **THEN** common namespaces (nirmata.Data.Dto.Models.Projects, nirmata.Services.Interfaces) are available without explicit @using directives
...
The nirmata.Web project SHALL serve static assets (CSS, JavaScript) from wwwroot with proper organization.
...
The nirmata.Web project SHALL provide a read-only list page displaying all projects.
...
The nirmata.Web project SHALL provide a read-only detail page for viewing a single project.
...
The nirmata.Web project SHALL provide a page for viewing workspace state and events.
...
The nirmata.Web project SHALL provide a page for managing checkpoints and execution continuity.
...
The nirmata.Web project SHALL provide a page for workspace validation and maintenance.
