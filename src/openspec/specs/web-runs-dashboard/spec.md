...
Defines the current implemented Runs UI in `nirmata.Web` for listing runs from the run repository and displaying per-run evidence/artifacts from `.aos/evidence/runs/**`.

- **Lives in:** `nirmata.Web/Pages/Runs/*`, `.aos/evidence/runs/<run-id>/**`
...
The `nirmata.Web` project SHALL provide a `/Runs` page that displays a list of all agent runs with their current status.
...
The `nirmata.Web` project SHALL provide a `/Runs/Details` page that displays comprehensive information about a single run.
