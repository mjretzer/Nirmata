...
Defines product data-layer conventions and behavior for $capabilityId.

- **Lives in:** `nirmata.Data/*`
- **Owns:** Persistence model conventions, EF Core configuration, and database behaviors
- **Does not own:** API surface contracts or agent/orchestration engine behavior
## Requirements
...
The data layer SHALL follow the established project layout: `nirmata.Data/Context` for DbContext, `nirmata.Data/Entities` for entities, `nirmata.Data/Mapping` for mappings, and `nirmata.Data/Migrations` for EF Core migrations.
...
The API configuration SHALL provide the SQLite connection string under `ConnectionStrings:DefaultConnection` and use a path rooted in the data project (e.g., `../nirmata.Data/sqllitedb/nirmata.db`).
...
The data layer SHALL implement `IDesignTimeDbContextFactory<nirmataDbContext>` in `nirmata.Data/Context` to construct the DbContext for EF Core tooling.

#### Scenario: Tooling context creation
- **WHEN** EF Core tooling runs outside the API host
- **THEN** it can construct `nirmataDbContext` via the design-time factory
