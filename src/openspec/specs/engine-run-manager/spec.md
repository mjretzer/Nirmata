# engine-run-manager Specification

## Purpose

Defines the public DI/interface surface for run lifecycle management in the AOS engine. Canonical run lifecycle semantics and evidence layout requirements are defined by `aos-run-lifecycle`, and this capability MUST conform.

- **Lives in:** `nirmata.Aos/Public/IRunManager.cs`, `nirmata.Aos/Public/Composition/*`, `.aos/evidence/runs/**`
- **Owns:** Public interface shape and DI registration for run lifecycle services
- **Does not own:** CLI command behavior or plane/orchestrator workflow control
## Requirements
### Requirement: Run manager interface exists
The system SHALL define `IRunManager` as a public interface in `nirmata.Aos/Public/`.
...
The system SHALL register `IRunManager` as a Singleton in `AddnirmataAos()`.
