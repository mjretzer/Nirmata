# engine-checkpoint-manager Specification

## Purpose

Defines the public DI/interface surface for checkpoint operations in the AOS engine. Canonical checkpoint semantics and CLI behavior are defined by `aos-checkpoints`, and this capability MUST conform.

- **Lives in:** `nirmata.Aos/Public/ICheckpointManager.cs`, `nirmata.Aos/Public/Composition/*`, `.aos/state/checkpoints/**`
- **Owns:** Public interface shape and DI registration for checkpoint create/restore/list
- **Does not own:** CLI command surface or higher-level plane workflow logic
## Requirements
### Requirement: Checkpoint manager interface exists
The system SHALL define `ICheckpointManager` as a public interface in `nirmata.Aos/Public/`.
...
The system SHALL register `ICheckpointManager` as a Singleton in `AddnirmataAos()`.
