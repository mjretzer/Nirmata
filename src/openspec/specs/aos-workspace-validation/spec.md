...
- each non-empty line MUST validate against the local event schema (`nirmata:aos:schema:event:v1`)
...
- **GIVEN** `.aos/state/events.ndjson` contains a non-empty line with `schemaVersion` that violates `nirmata:aos:schema:event:v1`
- **WHEN** `aos validate workspace` is executed
