...
- Read `nirmataAgents:SemanticKernel:Provider` configuration value
...
**New Approach:** Provider selection is handled by Semantic Kernel's connector configuration within `nirmataAgents:SemanticKernel` section.

#### Scenario: DI resolves provider from new Semantic Kernel configuration
- **GIVEN** configuration contains `nirmataAgents:SemanticKernel:Provider = "OpenAi"` and `nirmataAgents:SemanticKernel:OpenAi:ApiKey`
...
- **GIVEN** configuration does not contain `nirmataAgents:SemanticKernel:Provider`
...
- Load templates from embedded resources in `nirmata.Agents.Resources.Prompts`
