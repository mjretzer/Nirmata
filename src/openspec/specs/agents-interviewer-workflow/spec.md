...
The system SHALL provide an `INewProjectInterviewer` interface in `nirmata.Agents.Execution.Planning.NewProjectInterviewer` that conducts structured interviews to produce a valid project specification.

The interface MUST define:
- `ConductInterviewAsync(InterviewContext context, CancellationToken ct)` → returns `Task<InterviewResult>`
...
The system SHALL provide a `ProjectSpecGenerator` that converts interview session data into a valid `project.json` document conforming to schema `nirmata:aos:schema:project:v1`.
...
- **THEN** the output validates against `nirmata:aos:schema:project:v1` with the extracted values
