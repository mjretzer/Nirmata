# web-orchestrator-page Specification

## Purpose

Defines the current implemented Orchestrator UI in `nirmata.Web` for submitting commands/freeform input to the orchestration system and rendering structured results with safety rails context.

- **Lives in:** `nirmata.Web/Pages/Orchestrator/*`, `nirmata.Web/AgentRunner/WorkflowClassifier*`
- **Owns:** Chat-style command UX, message history persistence, and structured result rendering
- **Does not own:** Orchestrator business logic, run lifecycle semantics, or evidence schema contracts
## Requirements
### Requirement: Orchestrator page provides unified subagent command interface

The `nirmata.Web` project SHALL provide an `/Orchestrator` page that serves as the primary user interface for commanding subagents through the orchestration system.
...
- **THEN** the command is sent to the `IOrchestrator` via `WorkflowClassifier`
...
### Requirement: Orchestrator page integrates with WorkflowClassifier

The Orchestrator page model MUST use `WorkflowClassifier` for all orchestration requests.

The integration MUST:
- Inject `WorkflowClassifier` via constructor
...
- **GIVEN** a completed subagent execution via `WorkflowClassifier.ExecuteAsync()`
...
#### Scenario: Execute CLI command via WorkflowClassifier
...
#### Scenario: Execute freeform text via WorkflowClassifier
- **GIVEN** the Web application is configured with `AddnirmataAgents()`
- **WHEN** `WorkflowClassifier.ExecuteAsync("check the current state")` is called
