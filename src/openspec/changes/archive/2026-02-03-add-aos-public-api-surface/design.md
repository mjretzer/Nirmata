## Context
`nirmata.Aos` is the engine library that hosts workspace contracts, schemas/validation, deterministic IO, and orchestration primitives. Multiple projects (agents and hosts, and potentially product-facing surfaces later) need to *compile against* engine capabilities.
...
## Goals / Non-Goals
### Goals
- Make `nirmata.Aos.Public.*` the **only supported compile-against surface** for consumers.
- Keep `nirmata.Aos.Engine.*` and `nirmata.Aos._Shared.*` **internal-only** (implementation details).
- Provide a predictable place for stable contract types (`nirmata.Aos.Contracts.*`) used by the public surface (IDs, contract shapes).
...
We will keep one `nirmata.Aos` assembly and establish a strong convention:
- Public surface: `nirmata.Aos/Public/**` → `nirmata.Aos.Public.*`
- Contracts: `nirmata.Aos/Contracts/**` → `nirmata.Aos.Contracts.*`
- Internals: `nirmata.Aos/Engine/**` → `nirmata.Aos.Engine.*` (internal implementation)
- Internals: `nirmata.Aos/_Shared/**` → `nirmata.Aos._Shared.*` (internal shared helpers)
...
- **No public leakage**: no public types in `nirmata.Aos.Engine.*` / `nirmata.Aos._Shared.*`, and no public members (including generic type arguments) that reference internal types.
...
1) A unit test (reflection-based) that scans the built `nirmata.Aos` assembly for forbidden public API surface.
...
- Incrementally migrate any future consumers to compile against `nirmata.Aos.Public.*` only.

## Open Questions
- Should `nirmata.Aos.Contracts.*` be limited to ID catalogs + wire contracts only, or also include shared primitives used by both public and internal code?
