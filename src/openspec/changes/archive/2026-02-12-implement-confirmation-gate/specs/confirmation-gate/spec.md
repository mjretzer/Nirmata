...
## Purpose
Define the behavior of the confirmation gate system that intercepts write-side-effect operations and requires explicit user confirmation before proceeding, ensuring safe execution of destructive or ambiguous workflow actions.
...
The confirmation gate SHALL integrate prerequisite validation, confidence threshold evaluation, destructiveness analysis, and ProposedAction structure validation into the confirmation evaluation workflow.

#### Scenario: Complete confirmation evaluation workflow
- **GIVEN** the confirmation gate evaluates any write operation
- **WHEN** the evaluation proceeds
- **THEN** it SHALL first validate workspace prerequisites
...
#### Scenario: Prerequisite validation blocks confirmation
- **GIVEN** a write operation is being evaluated
- **AND** a required prerequisite (e.g., project spec) is missing
- **WHEN** the prerequisite validation runs
- **THEN** it SHALL return a conversational recovery action
- **AND** NOT proceed to confidence or destructiveness evaluation

#### Scenario: ProposedAction validation failure prevents confirmation
- **GIVEN** a write operation with an invalid ProposedAction (missing description)
- **WHEN** the ProposedAction structure is validated
- **THEN** the system SHALL reject the action
- **AND** emit an error event explaining the validation failure

## Cross-References

- Related to: `intent-classification` spec (confirmation requirements)
- Related to: `streaming-dialogue-protocol` (event definitions)
- Related to: `agents-orchestrator-workflow` (integration point)
- Related to: `aos-state-store` (persistence layer)
