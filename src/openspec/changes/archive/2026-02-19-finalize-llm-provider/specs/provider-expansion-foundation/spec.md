...
Defines the foundation for expanding LLM provider support beyond the baseline OpenAI implementation, establishing patterns and infrastructure for multi-provider support.

- **Lives in:** `nirmata.Agents/Execution/ControlPlane/Llm/*` and `nirmata.Agents/Configuration/*`
- **Owns:** Provider expansion patterns and infrastructure
- **Does not own:** Specific provider implementations (handled by future changes)
