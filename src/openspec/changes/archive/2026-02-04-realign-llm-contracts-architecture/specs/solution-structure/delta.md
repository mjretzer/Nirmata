...
- **AND** AOS does not have a project reference to nirmata.Agents

### Agent Plane (nirmata.Agents) concerns - LLM abstractions added

The Agent Plane (nirmata.Agents) section in `openspec/project.md` SHALL be updated to include LLM provider abstractions.
...
- **WHEN** the workflow uses ILlmProvider from nirmata.Agents.Contracts.Llm
...
The Dependency direction section SHALL be updated to reflect that nirmata.Agents references nirmata.Aos (not vice versa), and nirmata.Aos is LLM-agnostic.

Updated allowed references:
- `nirmata.Aos` → `nirmata.Common` (and internal engine dependencies only)
- `nirmata.Agents` → `nirmata.Aos`, `nirmata.Common`
...
- **NEW**: AOS must not reference nirmata.Agents (prevents circular dependency)
- AOS operates on evidence strings, not LLM types

#### Scenario: Agents depends on AOS for workspace services
- **GIVEN** nirmata.Agents implements an LLM-calling workflow
