## Context
The project has LLM provider contracts, adapters, and configuration in `nirmata.Aos`, but the architecture diagram (PDF) and clarified separation of concerns place these in `nirmata.Agents`. 

**Current confusion:**
- nirmata.Aos is meant to be the supporting engine (workspace, validation, state, evidence)
- nirmata.Agents is meant to be the orchestration plane (workflows that call LLMs)
...
- Make nirmata.Agents the owner of LLM orchestration concerns
- Make nirmata.Aos LLM-agnostic (only knows about evidence envelopes, not LLM types)
...
**What:** Contracts, adapters, configuration, and prompt resources all move to nirmata.Agents
...
- Create separate `nirmata.Llm` library → Rejected: overkill for current scope, Agents are the primary (only) consumer
...
- Preferred: Keep envelope as POCO with string properties, no reference to nirmata.Agents

### Decision 3: Dependency direction
**What:** nirmata.Agents references nirmata.Aos; nirmata.Aos does NOT reference nirmata.Agents
...
| Test files in wrong project | Audit nirmata.Aos.Tests before move; move LLM tests to new nirmata.Agents.Tests |
