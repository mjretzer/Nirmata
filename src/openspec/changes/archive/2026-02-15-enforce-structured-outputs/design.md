...
### 1. Schema Definition
We will define strict JSON schemas in `nirmata.Aos` for:
...
Modify `ILlmProvider` or its implementations in `nirmata.Agents` to support "Structured Output" or "Strict Mode" for tool schemas/response formats.
