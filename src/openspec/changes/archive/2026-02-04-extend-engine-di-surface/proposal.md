...
The Engine (`nirmata.Aos`) currently exposes 7 services via `AddnirmataAos()`:
...
- All new services are registered in `AddnirmataAos()` with appropriate lifetimes
- `dotnet build nirmata.Aos.csproj` passes
- Existing tests continue to pass
- Plane (`nirmata.Agents`) can resolve all services via DI
