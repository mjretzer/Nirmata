...
Each schema MUST:
- Include `$id` with canonical identifier format `nirmata:aos:schema:{artifact-type}:v{version}`
...
- **THEN** it returns a schema with `$id: "nirmata:aos:schema:phase-plan:v1"` and strict validation rules

#### Scenario: Registry defines unified task plan schema
- **GIVEN** the aos-schema-registry is initialized  
- **WHEN** querying for task plan schema
- **THEN** it returns a schema with `$id: "nirmata:aos:schema:task-plan:v1"` and canonical `fileScopes` object format

#### Scenario: Registry defines diagnostic artifact schema
- **GIVEN** the aos-schema-registry is initialized
- **WHEN** querying for diagnostic artifact schema
- **THEN** it returns a schema with `$id: "nirmata:aos:schema:diagnostic:v1"` with fields for validation errors, repair suggestions, and context
...
- `schemaId` (string): Always `"nirmata:aos:schema:diagnostic:v1"`
...
- **THEN** a diagnostic artifact is created with schema `nirmata:aos:schema:diagnostic:v1` containing specific errors and repair suggestions
...
The catalog MUST:
- Expose schema IDs via `nirmata.Aos.Public.Catalogs.SchemaIds`
