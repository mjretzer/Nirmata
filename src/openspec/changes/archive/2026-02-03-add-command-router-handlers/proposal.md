...
- **ADD** `ICommandRouter` interface with `RouteAsync` method in `nirmata.Aos/Public/Services/`
- **ADD** `CommandCatalog` for registering and resolving command handlers in `nirmata.Aos/Public/Catalogs/`
- **POPULATE** `CommandIds` with stable command identifiers for all core commands
- **ADD** `ICommandHandler` base interface and `CommandContext` in `nirmata.Aos/Engine/Commands/Base/`
- **ADD** Command group handlers in `nirmata.Aos/Engine/Commands/{Init,Status,Config,Validate,Spec,State,Runs}/`
- **ADD** Help renderer in `nirmata.Aos/Engine/Commands/Help/` that generates output from command catalog
- **ADD** `CommandRouter` implementation in `nirmata.Aos/Engine/Commands/`
...
- **Affected code:**
  - `nirmata.Aos/Public/Services/ICommandRouter.cs` (expanded from stub)
  - `nirmata.Aos/Public/Catalogs/CommandIds.cs` (populated)
  - `nirmata.Aos/Public/Catalogs/CommandCatalog.cs` (new)
  - `nirmata.Aos/Engine/Commands/` (new directory tree)
