...
- **Affected code:**
  - `nirmata.Aos/*` (run lifecycle, lock manager, secret store)
  - `nirmata.Agents/*` (continuity plane, rate limiting, secret injection)
  - `nirmata.Windows.Service/*` (background cleanup for abandoned runs)
  - `nirmata.Web/*` (UI status display for pause/resume)
