...
## Purpose
Defines rate limiting and concurrency bounds for orchestrator execution. Prevents unbounded parallel work and resource exhaustion.

- **Lives in:** `nirmata.Aos/*`, `.aos/**`, `nirmata.Agents/*`
- **Owns:** Concurrency configuration, task queue limits, LLM call rate limiting
- **Does not own:** Individual task implementation (owned by task executor)
