...
## Purpose
Defines crash-safe run finalization semantics. Unfinished runs are marked "abandoned" after a configurable timeout, enabling deterministic cleanup and recovery.

- **Lives in:** `nirmata.Aos/*`, `.aos/**`
- **Owns:** Run abandonment detection, timeout configuration, cleanup operations
...
The system SHALL provide a background task (in `nirmata.Windows.Service`) to periodically detect and mark abandoned runs.
