...
## Purpose
Defines secure credential storage, retrieval, and rotation. Eliminates plaintext secret storage and enables safe credential management.

- **Lives in:** `nirmata.Aos/*`, `nirmata.Agents/*`
- **Owns:** Secret store abstraction, OS keychain integration, configuration references
- **Does not own:** Individual provider implementations (owned by provider-specific capabilities)
