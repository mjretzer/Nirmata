...
The orchestrator is the control-plane heart of `nirmata.Agents`. It implements the "classify → gate → dispatch → validate → persist → next" workflow loop that determines what phase of work an agent should perform based on workspace state.

Current state:
- `nirmata.Aos` provides public APIs: `ICommandRouter`, `IWorkspace`, `ISpecStore`, `IStateStore`, `IValidator`
- `nirmata.Agents` has basic `IRunRepository` and `RunRepository` (in-memory stub)
...
- **Rationale**: `nirmata.Agents` shouldn't directly access Engine internals; `IRunRepository` provides the boundary
...
- **Risk**: Tight coupling to AOS public API surface → Mitigation: Only use interfaces from `nirmata.Aos.Public`
