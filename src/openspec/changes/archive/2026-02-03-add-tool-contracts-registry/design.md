...
**Chosen:** `nirmata.Aos/Mcp/**` namespace for MCP-specific adapter types
...
**Rationale:** Matches existing error patterns in nirmata.Common; serializable for evidence capture
...
- **Tool ID collision** → Mitigation: ToolIds catalog uses `nirmata:aos:tool:` prefix; external tools use `mcp:` prefix
