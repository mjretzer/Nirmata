## Context
nirmata.Web is the user-facing web UI for the nirmata platform. Currently it has only a minimal Program.cs stub. This change establishes the Razor Pages foundation with a clean architecture that separates page models (logic) from views (presentation).
...
- **Service Integration**: Inject `IProjectService` directly into page models (allowed per project.md: nirmata.Web → nirmata.Services reference is valid)
