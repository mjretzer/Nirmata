...
Defines the tool execution infrastructure for the Agent Plane (`nirmata.Agents`), including tool registry, descriptors, and MCP adapter support. This capability works alongside the LLM provider abstractions to enable tool discovery and invocation during workflow orchestration.
