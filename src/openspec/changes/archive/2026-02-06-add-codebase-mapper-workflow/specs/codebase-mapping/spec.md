...
## Purpose
Define the Codebase Mapper workflow that produces an `.aos/codebase/**` intelligence pack. The codebase intelligence pack provides grounded repository understanding for brownfield planning, including structure detection, symbol indexing, file dependency graphs, and architectural conventions.
...
- **GIVEN** a codebase with a type `nirmata.Agents.ICodebaseScanner`
...
## ADDED Relationships
- **relates-to:** `aos-deterministic-json-serialization` — codebase artifacts use deterministic JSON writer
- **relates-to:** `aos-schema-registry` — all codebase schemas registered
- **relates-to:** `aos-workspace-validation` — codebase pack validation rules
- **enables:** `agents-brownfield-planner` — future brownfield planning workflow (not yet defined)
