...
## Purpose
Provides a web interface for viewing and managing milestones, including milestone details with associated phases, status tracking, and completion gates.

## ADDED Requirements

### Requirement: Milestones List Page
The `nirmata.Web` project SHALL provide a `/Milestones` page that displays all milestones with their status and phases.

The implementation MUST:
- Read milestones from `.aos/spec/milestones/index.json` and individual milestone files
...
- Link each milestone to its detail page (`/Milestones/Details?id={milestoneId}`)
...
The `nirmata.Web` project SHALL provide a `/Milestones/Details` page that displays comprehensive information about a single milestone.

The implementation MUST:
- Accept milestone ID via query parameter (`?id={milestoneId}`)
...
- Link each phase to its detail page (`/Phases/Details?id={phaseId}`)
...
- **WHEN** a user navigates to `/Milestones/Details?id=MS-0001`
...
- **WHEN** a user navigates to `/Milestones/Details?id=nonexistent`
...
- Generate the next sequential milestone ID (MS-#### format)
- Write milestone to `.aos/spec/milestones/{id}/milestone.json`
- Update `.aos/spec/milestones/index.json`
- Update `.aos/spec/roadmap.json` to include the new milestone
...
- **THEN** a new milestone MS-0002 is created, the index is updated, and the user is redirected to `/Milestones/Details?id=MS-0002`
