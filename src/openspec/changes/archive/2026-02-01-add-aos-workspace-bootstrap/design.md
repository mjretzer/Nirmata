...
- Keep the workspace bootstrap business logic and the `aos init` command handler in `nirmata.Aos` (engine-owned), with no Product Application dependencies.
...
## Open Questions
- Exact packaging of the CLI as `aos` (local dotnet tool vs app in solution) for developer ergonomics, while keeping command logic owned by `nirmata.Aos`.
