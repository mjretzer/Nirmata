...
- [x] 2.1 Add an engine workspace bootstrapper in `nirmata.Aos` that creates/verifies the `.aos/*` skeleton and writes baseline artifacts.
- [x] 2.2 Implement the `aos init` command wiring (argument parsing + handler) in `nirmata.Aos` and invoke the bootstrapper at repository root.
