...
This change builds on the existing `nirmata.Web/ChatStreamingController` SSE infrastructure and the `nirmata.Agents` orchestrator. It requires coordination with `redesign-ui-chat-forward` for UI rendering but is primarily a backend protocol change.

**Dependencies**:
- `redesign-ui-chat-forward` Phase 1 complete (basic streaming infrastructure exists)
- `nirmata.Agents` orchestrator with classification and gating components
