...
The engine already ships an embedded schema pack (`nirmata.Aos/Resources/Schemas/**`) and seeds a local schema pack under `.aos/schemas/**` during `aos init`. However, workspace validation largely stops at “valid JSON” checks; it does not validate artifacts against JSON Schema.
