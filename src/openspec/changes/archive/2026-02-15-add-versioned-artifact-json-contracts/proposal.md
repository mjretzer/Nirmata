...
- Schema and registry alignment:
  - Add/update schema pack entries for `nirmata.task-plan` and `nirmata.command-proposal`.
...
- Affected code (expected during apply stage):
  - `nirmata.Aos` schema registry + validator/reporting components
  - `nirmata.Agents` planning/execution/fix-planning models and writers/readers
