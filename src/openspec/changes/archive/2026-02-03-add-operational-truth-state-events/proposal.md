...
- Tighten workspace validation for `.aos/state/events.ndjson`:
  ...
  - each event line MUST validate against the local event schema (`nirmata:aos:schema:event:v1`), while keeping that schema permissive (only `schemaVersion` required).
...
- **Code paths (implementation stage)**:
  - `nirmata.Aos/Engine/Stores/AosStateStore.cs`
  - `nirmata.Aos/Engine/Validation/AosWorkspaceValidator.cs`
  - `nirmata.Aos/Resources/Schemas/state-snapshot.schema.json`
  - `nirmata.Aos/Public/**` and `nirmata.Aos/Contracts/**`
- **Tests (implementation stage)**:
  - `tests/nirmata.Aos.Tests/**` (state store determinism, event tail/filter, workspace validation)
