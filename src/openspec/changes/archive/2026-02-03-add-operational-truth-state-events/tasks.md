...
- [x] 2.1 Introduce stable contract DTOs under `nirmata.Aos/Contracts/State/**` for state snapshot + event entries and filter options.
...
- [x] 4.1 Flesh out `nirmata.Aos.Public` state store interface so consumers can read snapshots, append events, and tail/filter events.
- [x] 4.2 Ensure internal engine types remain internal (public APIs exchange only `nirmata.Aos.Contracts.*` types).
