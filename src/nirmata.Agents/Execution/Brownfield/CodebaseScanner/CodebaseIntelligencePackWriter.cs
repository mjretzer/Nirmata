...
public sealed class CodebaseIntelligencePackWriter
{
    ...
    /// </summary>
    private static Task WriteHashManifestAsync(string path, Dictionary<string, string> hashes, DateTimeOffset timestamp, CancellationToken ct)
    {
        ...
        // Use DeterministicJsonFileWriter to ensure consistent formatting and trailing newline
        DeterministicJsonFileWriter.WriteCanonicalJsonOverwrite(path, manifestData, new JsonSerializerOptions
        {
            PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
            WriteIndented = true
        });
    ...
    public static async Task<(bool IsValid, List<string> Mismatches)> VerifyDeterminismAsync(string outputDirectory, CancellationToken ct = default)
    {
        ...
        // Use camelCase policy to correctly match "files" property
        var manifest = JsonSerializer.Deserialize<HashManifest>(manifestJson, new JsonSerializerOptions 
        { 
            PropertyNamingPolicy = JsonNamingPolicy.CamelCase 
        });
