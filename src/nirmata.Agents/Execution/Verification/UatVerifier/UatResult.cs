...
public sealed class UatResultWriter : IUatResultWriter
{
    ...
    public async Task<string> WriteResultAsync(string taskId, string runId, UatVerificationResult result, CancellationToken ct = default)
    {
        ...
        {
            SchemaVersion = "nirmata:aos:schema:uat-result:v1",
            RunId = runId,
            TaskId = taskId,
        ...
        // Validate verifier output before writing
        var validation = nirmata.Agents.Execution.Validation.ArtifactContractValidator.ValidateVerifierOutput(
            artifactPath: artifactPath,
            artifactJson: json,
            aosRootPath: _workspace.AosRootPath,
            readBoundary: "verifier-writer");

        if (!validation.IsValid)
        {
            // We log but still write the file to avoid losing data, as verification results are critical
            // and downstream components might still be able to use parts of it.
            // In a strict mode, we might throw here.
        }
