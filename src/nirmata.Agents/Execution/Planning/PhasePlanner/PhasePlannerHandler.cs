...
public sealed class PhasePlannerHandler
{
    ...
    public async Task<PhasePlannerResult> PlanPhaseAsync(string phaseId, string runId, CancellationToken ct = default)
    {
        ...
        {
            // Step 1: Gather context for the phase
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Gathering context for {phaseId}");
            var brief = await _contextGatherer.GatherContextAsync(phaseId, runId, ct);
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Context gathered. Brief is null? {brief == null}");
            ...
            if (brief == null)
            {
                return new PhasePlannerResult
                {
                    IsSuccess = false,
                    PhaseId = phaseId,
                    ErrorMessage = "Failed to gather phase context (brief is null)",
                    StartedAt = startedAt,
            ...
            // Step 2: Create task plan from the brief
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Creating task plan");
            var taskPlan = await _phasePlanner.CreateTaskPlanAsync(brief, runId, ct);
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Task plan created. IsValid? {taskPlan.IsValid}");
            ...
            // Step 3: Extract and document assumptions
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Extracting assumptions");
            var assumptions = await _assumptionLister.ExtractAssumptionsAsync(brief, taskPlan, runId, ct);
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Assumptions extracted. Count: {assumptions?.Count}");
            
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Generating assumptions document");
            string assumptionsPath = string.Empty;
            if (assumptions != null)
            {
                assumptionsPath = await _assumptionLister.GenerateAssumptionsDocumentAsync(assumptions, runId, ct);
            }
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Assumptions document path: {assumptionsPath}");

            // Step 4: Persist planning decision to events.ndjson
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Persisting planning decision");
            ...
            return new PhasePlannerResult
            {
                ...
                PhaseBrief = brief,
                TaskPlan = taskPlan,
                Assumptions = assumptions ?? Array.Empty<PhaseAssumption>(),
        ...
        {
            Console.WriteLine($"DEBUG: PhasePlannerHandler - Exception caught: {ex.GetType().Name} - {ex.Message}\n{ex.StackTrace}");
            return new PhasePlannerResult
            {
                IsSuccess = false,
                PhaseId = phaseId,
