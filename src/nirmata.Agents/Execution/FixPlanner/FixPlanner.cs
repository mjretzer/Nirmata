...
namespace nirmata.Agents.Execution.FixPlanner;
...
{
    private const string FixPlanSchemaResourceName = "nirmata.Aos.Resources.Schemas.fix-plan.schema.json";
    ...
    private static readonly Lazy<LlmStructuredOutputSchema> StructuredFixPlanSchema = new(() =>
        LlmStructuredOutputSchema.FromJson(
            ...
            description: "nirmata fix planning schema"),
        LazyThreadSafetyMode.ExecutionAndPublication);
        ...
        CancellationToken ct)
    {
        ...
        var validation = nirmata.Agents.Execution.Validation.ArtifactContractValidator.ValidateFixPlan(
    ...
    private void CreateDiagnosticForLlmValidationFailure(FixPlannerRequest request, LlmProviderException ex)
    {
        try
        {
            ...
            var diagnostic = new DiagnosticArtifact
            {
                ...
                FailedSchemaId = "nirmata:aos:schema:fix-plan:v1",
