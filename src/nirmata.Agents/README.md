...
Agent orchestration and execution layer for the nirmata platform. Implements workflow management, subagent coordination, task execution, and tool calling protocols.

## Overview

nirmata.Agents provides the execution engine for nirmata workflows:
...
```csharp
// Program.cs
builder.Services.AddnirmataAgents(builder.Configuration);
```
...
Ensure the 'nirmataAgents:SemanticKernel' section is configured in appsettings.json.
...
| Error | Cause | Solution |
|-------|-------|----------|
| "configuration is missing" | `nirmataAgents:SemanticKernel` section not found | Add section to appsettings.json |
...
{
  "nirmataAgents": {
    "WorkspacePath": "C:/nirmata/Workspace",
    "ToolCalling": {
...
- `nirmata.Aos` - AOS (Agent Operating System) contracts and state management
- `nirmata.Common` - Shared utilities and exceptions
