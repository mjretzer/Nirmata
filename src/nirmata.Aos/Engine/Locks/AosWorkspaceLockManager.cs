...
namespace nirmata.Aos.Engine.Locks;
        ...
        try
        {
            ...
            try
            {
                ...
                return AosWorkspaceLockAcquireResult.CreateAcquired(
                    new AosWorkspaceLockHandle(lockPath, lockId, fs)
                );
            }
            catch
            {
...
{
    private readonly FileStream? _heldHandle;
    public string LockPath { get; }
    public string LockId { get; }

    public AosWorkspaceLockHandle(string lockPath, string lockId, FileStream? heldHandle = null)
    {
    ...
    public void Dispose()
    {
        try
        {
            if (_heldHandle != null)
            {
                // Atomic release: closing the handle that holds FileShare.None
                // allows others to acquire it immediately after deletion.
                _heldHandle.Dispose();
            }
            ...
            // If we have the handle, we trust our identity because we held it exclusively.
            // If we don't have the handle (e.g. force release scenarios if they use handle), 
            // we fall back to identity check.
            if (_heldHandle == null)
            {
