...
public interface IRunManager
{
    ...
    /// <summary>
    /// Marks a run as abandoned after a timeout period.
    /// </summary>
    /// <param name="runId">The run ID to mark as abandoned.</param>
    /// <param name="abandonedAtUtc">The timestamp when the run was abandoned.</param>
    /// <exception cref="ArgumentException">Thrown when runId is invalid.</exception>
    /// <exception cref="FileNotFoundException">Thrown when run metadata is not found.</exception>
    /// <remarks>
    /// Updates run.json status to "abandoned" and updates the run index.
    /// Only marks runs that are in "started" status; finished runs are not modified.
    /// </remarks>
    void MarkRunAbandoned(string runId, DateTimeOffset abandonedAtUtc);

    /// <summary>
    /// Pauses a run, updating its status to "paused".
    /// </summary>
    /// <param name="runId">The run ID to pause.</param>
    /// <param name="pausedAtUtc">The timestamp when the run was paused.</param>
    /// <exception cref="ArgumentException">Thrown when runId is invalid.</exception>
    /// <exception cref="FileNotFoundException">Thrown when run metadata is not found.</exception>
    /// <exception cref="InvalidOperationException">Thrown when run is not in "started" status.</exception>
    /// <remarks>
    /// Updates run.json status to "paused" and updates the run index.
    /// Only pauses runs that are in "started" status.
    /// </remarks>
    void PauseRun(string runId, DateTimeOffset pausedAtUtc);

    /// <summary>
    /// Resumes a paused run, updating its status back to "started".
    /// </summary>
    /// <param name="runId">The run ID to resume.</param>
    /// <param name="resumedAtUtc">The timestamp when the run was resumed.</param>
    /// <exception cref="ArgumentException">Thrown when runId is invalid.</exception>
    /// <exception cref="FileNotFoundException">Thrown when run metadata is not found.</exception>
    /// <exception cref="InvalidOperationException">Thrown when run is not in "paused" status.</exception>
    /// <remarks>
    /// Updates run.json status back to "started" and updates the run index.
    /// Only resumes runs that are in "paused" status.
    /// </remarks>
    void ResumeRun(string runId, DateTimeOffset resumedAtUtc);
}
