...
This document provides provider-specific guidance for integrating LLM structured output schemas with nirmata planners and verifiers.
...
- The nirmata retry handler catches this and retries up to 3 times with enhanced prompts
...
- nirmata validates the response using `JsonSchema.Net` after receiving it
