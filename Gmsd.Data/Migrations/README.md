...
This directory contains database migration tooling and seed data for the GMSD application.
...
Seed data is configured in `GmsdDbContext.OnModelCreating()` using EF Core's `HasData()` method. The following entities are seeded on fresh database installations:
...
- `GmsdDbContext.OnConfiguring()` (design-time fallback)
