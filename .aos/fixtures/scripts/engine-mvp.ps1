...
dotnet build (Join-Path $repoRoot 'Gmsd.Aos/Gmsd.Aos.csproj') -c $Configuration | Out-Host

$aosDll = Join-Path $repoRoot "Gmsd.Aos/bin/$Configuration/net10.0/aos.dll"
...
if ([string]::IsNullOrWhiteSpace($workspaceRoot)) {
    $workspaceRoot = New-TempDirectory 'gmsd-aos-engine-mvp'
