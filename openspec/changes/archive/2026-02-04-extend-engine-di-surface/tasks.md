...
- [x] Define `IArtifactPathResolver` interface in `Gmsd.Aos/Public/`
- [x] Map artifact IDs (MS/PH/TSK/ISS/UAT/PCK/RUN) to canonical paths
- [x] Register as Singleton in `AddGmsdAos()`
...
- [x] Extend `AddGmsdAos()` to register all 8 new services
...
- [x] `dotnet build Gmsd.Aos.csproj` passes
- [x] `dotnet build Gmsd.Agents.csproj` passes (can resolve services)
