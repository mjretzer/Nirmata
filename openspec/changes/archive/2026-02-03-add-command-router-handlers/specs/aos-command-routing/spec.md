...
Define the command routing system for the AOS engine, enabling deterministic routing of `{group, command}` requests to registered handlers with structured error responses and evidence integration.

## ADDED Requirements

### Requirement: Public command router interface

The system SHALL provide a public `ICommandRouter` interface in `Gmsd.Aos.Public.Services` that accepts a `CommandRequest` and returns a `Task<CommandRouteResult>`.
...
The system SHALL provide a `CommandCatalog` in `Gmsd.Aos.Public.Catalogs` that allows registration of command handlers by `{group, command}` tuple.
...
The system SHALL provide stable command identifiers in `Gmsd.Aos.Public.Catalogs.CommandIds` for all core commands:
...
The system SHALL provide an `ICommandHandler` interface in `Gmsd.Aos.Engine.Commands.Base` that all command handlers implement.
