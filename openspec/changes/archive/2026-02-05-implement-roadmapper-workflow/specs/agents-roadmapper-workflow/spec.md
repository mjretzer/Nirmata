...
The system SHALL provide an `IRoadmapper` interface in `Gmsd.Agents.Execution.Planning.Roadmapper` that generates a complete roadmap from a validated project specification.
...
- Validate output against `gmsd:aos:schema:milestone:v1` before returning
...
- **THEN** the output validates against `gmsd:aos:schema:milestone:v1` with `id: "MS-0001"` and `name: "MyApplication Delivery"`
...
- Validate against `gmsd:aos:schema:phase:v1`
...
- Validate against `gmsd:aos:schema:event:v1`
