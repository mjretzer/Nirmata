...
## Goals / Non-Goals
### Goals
- Provide a schema registry that loads JSON Schemas shipped with the engine so validation is consistent and versioned with `Gmsd.Aos`.
...
- Keep all implementation engine-owned (`Gmsd.Aos`) with no Product Application dependencies.
...
## Decisions
### Decision: Shipped schemas are embedded resources owned by `Gmsd.Aos`
