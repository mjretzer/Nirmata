...
## What Changes
- Add a **JSON Schema registry loader** in `Gmsd.Aos` that loads schemas shipped with the engine (embedded resources) so validation is consistent and versioned with the library.
...
- **Affected code**:
  - `Gmsd.Aos` CLI command routing and new validation components
  - new embedded schema assets shipped with `Gmsd.Aos`
