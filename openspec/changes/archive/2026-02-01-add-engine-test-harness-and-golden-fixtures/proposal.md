...
- **Affected code (later apply stage)**:
  - Test harness and fixtures under `tests/Gmsd.Aos.Tests/Fixtures/**`
...
## Notes
- Versioned fixtures are stored under `tests/Gmsd.Aos.Tests/Fixtures/**` (source of truth), following the existing approved-fixture pattern used by `AosInitFixtureTests`.
