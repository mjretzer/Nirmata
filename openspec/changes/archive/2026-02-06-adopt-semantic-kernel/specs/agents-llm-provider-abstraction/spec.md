...
Template IDs map to embedded resource paths in `Gmsd.Agents.Resources.Prompts` with `.prompt.txt` or `.prompt.yaml` extensions.
...
- **GIVEN** a workflow in Gmsd.Agents executes a chat completion via SK
