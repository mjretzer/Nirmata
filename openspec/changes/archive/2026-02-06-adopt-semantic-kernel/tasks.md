...
- [x] 1.1 Add Semantic Kernel packages to `Gmsd.Agents.csproj`
  ...
  - Verify `dotnet build Gmsd.Agents.csproj` passes

- [x] 1.2 Update `Gmsd.Agents/Composition/ServiceCollectionExtensions.cs`
  - Add `using Microsoft.SemanticKernel;`
  - Keep existing `AddGmsdAgents()` method for backward compatibility

## 2. Semantic Kernel Infrastructure
- [x] 2.1 Create `Gmsd.Agents/Configuration/SemanticKernelOptions.cs`
...
- [x] 2.2 Create `Gmsd.Agents/Configuration/SemanticKernelServiceCollectionExtensions.cs`
...
- [x] 2.3 Create `Gmsd.Agents/Execution/ControlPlane/Llm/Filters/AosEvidenceFunctionFilter.cs`
...
- [x] 3.1 Create `Gmsd.Agents/Execution/ControlPlane/Llm/Tools/ToolToKernelFunctionAdapter.cs`
...
- [x] 3.2 Create `Gmsd.Agents/Execution/ControlPlane/Llm/Tools/KernelPluginFactory.cs`
...
- [x] 4.1 Create `Gmsd.Agents/Execution/ControlPlane/Llm/Prompts/SemanticKernelPromptFactory.cs`
  - Implement `KernelFunction CreateFromEmbeddedResource(string resourceId)`
  - Load embedded resources from `Gmsd.Agents.Resources.Prompts`
...
- [x] 4.2 Review and update existing prompt files for SK compatibility
  - Migrated prompt from `Gmsd.Aos` to `Gmsd.Agents/Resources/Prompts`
...
- [x] 5.3 Implement or configure Ollama connector
  ...
  - Tested with `dotnet build Gmsd.Agents.csproj` - build successful
...
- [x] 8.1 Verify no compile-time errors in `Gmsd.Agents`
  - Run `dotnet build Gmsd.Agents.csproj`
  - Run `dotnet build Gmsd.slnx`
