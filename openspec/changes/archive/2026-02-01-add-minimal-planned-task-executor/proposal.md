...
- Organize implementation for separation of concerns:
  - plan loading + execution under `Gmsd.Aos/Engine/ExecutePlan/**`
  - evidence writing helpers under `Gmsd.Aos/Engine/Evidence/ExecutePlan/**`

## Impact
- **Affected specs**:
  - new capability `aos-execute-plan`
- **Affected code** (expected during implementation):
  - `Gmsd.Aos` CLI command routing (`aos execute-plan ...`)
  - plan loading + validation under `Gmsd.Aos/Engine/ExecutePlan/**`
  - evidence writing code under `Gmsd.Aos/Engine/Evidence/ExecutePlan/**`
