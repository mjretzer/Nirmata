...
- **Affected code (expected implementation touchpoints)**:
  - `Gmsd.Aos/Engine/Workspace/AosWorkspaceBootstrapper.cs` (init JSON templates + registry)
  - `Gmsd.Aos/Engine/Evidence/Runs/AosRunEvidenceScaffolder.cs` (run.json + runs/index.json)
  - `Gmsd.Aos/Engine/Evidence/ExecutePlan/ExecutePlanActionsLogWriter.cs` (execute-plan.actions.json)
  - New shared writer utility under `Gmsd.Aos` engine (canonicalize + atomic/no-churn write)
- **Risks**:
