...
The system SHALL maintain a `ToolIds` catalog class in `Gmsd.Aos.Public.Catalogs` containing constants for built-in tool identifiers.

Tool ID constants MUST follow the pattern: `gmsd:aos:tool:{category}:{name}:{version}`
...
- **THEN** they receive the stable identifier `"gmsd:aos:tool:filesystem:read:v1"`
