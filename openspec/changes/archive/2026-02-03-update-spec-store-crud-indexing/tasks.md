...
- [x] 1.1 Update routing contract paths for Issue/UAT
  - [x] Update `Gmsd.Aos/Engine/Paths/AosPathRouter.cs` Issue/UAT paths to `.aos/spec/issues/<id>.json` and `.aos/spec/uat/<id>.json`
  - [x] Update routing tests (e.g., `tests/Gmsd.Aos.Tests/AosPathRouterTests.cs`)

- [x] 1.2 Update SpecStore CRUD surface
  - [x] Add CRUD methods in `Gmsd.Aos/Engine/Stores/AosSpecStore.cs` for:
...
- [x] 1.3 Update index repair for Issue/UAT discovery
  - [x] Update `Gmsd.Aos/Engine/Repair/AosIndexRepairer.cs` to rebuild Issue/UAT indexes from flat `*.json` artifacts under:
    - `.aos/spec/issues/*.json` (excluding `index.json`)
    - `.aos/spec/uat/*.json` (excluding `index.json`)
  - [x] Update repair determinism tests (e.g., `tests/Gmsd.Aos.Tests/AosRepairIndexesDeterminismTests.cs`)
...
- [ ] 2.1 Update public surface contracts if needed
  - [ ] If SpecStore is intended to be consumer-facing now, define minimal `Gmsd.Aos.Public.ISpecStore` methods and stable contract types under `Gmsd.Aos.Contracts` (otherwise keep internal and defer as follow-up)
