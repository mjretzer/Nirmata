...
## Purpose
Define cross-cutting UI components that provide consistent UX patterns across all GMSD Web pages: global command palette, workspace context visibility, artifact linking, and toast notifications.

## ADDED Requirements

### Requirement: Global Command Palette
The Gmsd.Web project SHALL provide a global command palette accessible via keyboard shortcut.
...
- **GIVEN** any GMSD Web page is loaded
...
The Gmsd.Web project SHALL display a persistent workspace badge in the header showing current context.
...
- **WHEN** any page loads
- **THEN** the header shows the workspace path (e.g., "GMSD")
...
The Gmsd.Web project SHALL automatically convert artifact references to clickable links.
...
The Gmsd.Web project SHALL provide a toast notification system for user feedback.
...
The Gmsd.Web project SHALL ensure all cross-cutting components share consistent styling.
