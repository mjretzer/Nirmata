...
- [x] **TASK-001**: Create `FakeValidator` utility with reflection-based interface compliance checking
  - **Scope**: `tests/Gmsd.Agents.Tests/Contracts/FakeValidator.cs`
...
- [x] **TASK-005**: Add contract tests for all AOS fakes
  - **Scope**: `tests/Gmsd.Agents.Tests/Contracts/AosFakeContractTests.cs`
...
- [x] **TASK-006**: Create `FakeSymbolCacheBuilder`
  - **Scope**: `tests/Gmsd.Agents.Tests/Fakes/Agents/FakeSymbolCacheBuilder.cs`
...
- [x] **TASK-009**: Add contract tests for agent fakes
  - **Scope**: `tests/Gmsd.Agents.Tests/Contracts/AgentsFakeContractTests.cs`
...
- [x] **TASK-010**: Create `AosTestWorkspaceBuilder`
  - **Scope**: `tests/Gmsd.Agents.Tests/Fixtures/AosTestWorkspaceBuilder.cs`
...
- [x] **TASK-011**: Create `HandlerTestHost` DI container
  - **Scope**: `tests/Gmsd.Agents.Tests/Fixtures/HandlerTestHost.cs`
...
- [x] **TASK-012**: Create `AutoFakeBuilder` for handler mocks
  - **Scope**: `tests/Gmsd.Agents.Tests/Fixtures/AutoFakeBuilder.cs`
...
- [x] **TASK-013**: Rewrite `OrchestratorEndToEndTests`
  - **Scope**: `tests/Gmsd.Agents.Tests/Integration/Orchestrator/OrchestratorEndToEndTests.cs`
...
- [x] **TASK-014**: Rewrite `GatingEngineIntegrationTests`
  - **Scope**: `tests/Gmsd.Agents.Tests/Integration/Orchestrator/GatingEngineIntegrationTests.cs`
...
- [x] **TASK-015**: Fix namespace issues in remaining test files
  - **Scope**: Update all `using Gmsd.Agents.Workflows` → `using Gmsd.Agents.Execution`
...
- [x] **TASK-019**: Document test patterns
  - **Scope**: `tests/Gmsd.Agents.Tests/README.md`
  - **Verify**: Examples for: Contract test, Handler test, E2E test
  - **Time**: 1 hour

- [x] **TASK-020**: Create test template
  - **Scope**: `tests/Gmsd.Agents.Tests/Templates/HandlerTestTemplate.cs`
...
- [x] `dotnet build tests/Gmsd.Agents.Tests/Gmsd.Agents.Tests.csproj` → 0 errors
- [x] `dotnet build` (entire solution) → 0 errors (blocked by running Gmsd.Web process locking files - not a code issue)
...
- [x] No references to `Gmsd.Agents.Workflows` namespace
...
- All work contained within `tests/Gmsd.Agents.Tests/`
