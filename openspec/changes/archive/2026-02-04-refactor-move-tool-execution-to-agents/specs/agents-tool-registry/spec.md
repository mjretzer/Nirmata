...
Defines the tool execution infrastructure for the Agent Plane (`Gmsd.Agents`), including tool registry, descriptors, and MCP adapter support. This capability works alongside the LLM provider abstractions to enable tool discovery and invocation during workflow orchestration.

## ADDED Requirements

### Requirement: Tool descriptors expose metadata for introspection
The system SHALL define a `ToolDescriptor` model containing:
- `string Id` - stable unique identifier (e.g., `gmsd:aos:tool:filesystem:read`)
...
Tool ID constants MUST follow the pattern: `gmsd:aos:tool:{category}:{name}`
...
- **THEN** they receive the stable identifier `"gmsd:aos:tool:filesystem:read"`
...
- **THEN** it returns `"gmsd:aos:tool:mcp:filesystem:read"`
