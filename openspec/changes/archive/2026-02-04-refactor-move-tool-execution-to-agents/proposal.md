...
The architectural boundary between `Gmsd.Aos` (Engine) and `Gmsd.Agents` (Plane) is currently violated by having tool execution infrastructure in the wrong layer. According to `@/openspec/project.md` and `@/openspec/roadmap.md`:
...
These should be in `Gmsd.Agents` alongside LLM provider abstractions, as they represent tool *execution* infrastructure, not workspace primitives.

## What Changes

### Code Moves

**Move from `Gmsd.Aos` to `Gmsd.Agents`:**
- `Gmsd.Aos/Tools/` → `Gmsd.Agents/Tools/`
  - `ToolDescriptor.cs`
  - `ToolCapability.cs` 
  - `ToolParameter.cs`
- `Gmsd.Aos/Mcp/` → `Gmsd.Agents/Mcp/`
  - `McpToolAdapter.cs`
  - `McpToolFactory.cs`
- `Gmsd.Aos/Registry/` → `Gmsd.Agents/Registry/`
  - `IToolRegistry.cs`
  - `ToolRegistry.cs`

**Keep in `Gmsd.Aos/Contracts/Tools/`:**
...
- `Gmsd.Aos.Mcp` → `Gmsd.Agents.Mcp`
- `Gmsd.Aos.Registry` → `Gmsd.Agents.Registry`
- `Gmsd.Aos.Contracts.Tools` (unchanged - already correct)
