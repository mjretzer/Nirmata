...
namespace Gmsd.Agents.Tests.Execution.CodebaseScanner;
...
public class CodebaseScannerTests
{
    ...
    public async Task ScanAsync_DiscoversSolutions()
    {
        ...
        result.Solutions.Should().Contain(s => s.Name == "Gmsd" || s.Name.Contains("Gmsd"));
    }
    ...
    public async Task ScanAsync_DiscoversProjects()
    {
        ...
        result.Projects.Should().Contain(p => p.Name.Contains("Gmsd.Agents") || p.Name.Contains("Gmsd.Aos"));
    }
    ...
    public async Task ScanAsync_WithInvalidPath_ReturnsFailure()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<DirectoryNotFoundException>(() => _scanner.ScanAsync(request));
    }
    ...
    public async Task ScanAsync_ProjectHasCorrectReferences()
    {
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        var agentsProject = result.Projects.FirstOrDefault(p => p.Name == "Gmsd.Agents");
