...
public class CodebaseScannerBenchmarkTests
{
    ...
    public async Task Benchmark_IncrementalScan_Performance()
    {
        ...
        // Incremental should generally be faster when nothing changed
        incrementalMs.Should().BeLessThan((long)(fullScanMs * 1.2)); // Allow some overhead
    }
    ...
    public async Task Benchmark_Determinism_ByteIdenticalOutput()
    {
        // Arrange
        var repositoryRoot = GetRepositoryRoot();
        var tempDir = Path.Combine(Path.GetTempPath(), $"gmsd-determinism-test-{Guid.NewGuid():N}");
