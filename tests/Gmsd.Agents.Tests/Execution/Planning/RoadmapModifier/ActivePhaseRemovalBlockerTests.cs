...
public class ActivePhaseRemovalBlockerIntegrationTests : IDisposable
{
    ...
    public ActivePhaseRemovalBlockerIntegrationTests()
    {
        ...
        _specStore = SpecStore.FromAosRoot(_tempDirectory);
    ...
    public async Task RemoveActivePhase_WithoutForce_ReturnsBlockedResult()
    {
        ...
        // Assert
        result.IsBlocked.Should().BeTrue("Should be blocked when removing active phase without force");
    ...
    public async Task RemoveActivePhase_WithoutForce_CreatesBlockerIssue()
    {
        ...
        if (Directory.Exists(issuesDir))
        {
            ...
            if (issueFiles.Length > 0)
            {
                ...
                issueDoc.RootElement.GetProperty("issueId").GetString().Should().Be(result.BlockerIssueId);
