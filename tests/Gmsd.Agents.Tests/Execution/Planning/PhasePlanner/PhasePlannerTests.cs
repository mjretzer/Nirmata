...
public class PhasePlannerTests
{
    ...
    public async Task CreateTaskPlanAsync_WithNullRunId_ThrowsArgumentException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ArgumentException>(() => _sut.CreateTaskPlanAsync(brief, null!));
    }
    ...
    public async Task CreateTaskPlanAsync_WithZeroTasks_ReturnsInvalidPlan()
    {
        ...
        // Assert
        result.IsValid.Should().BeFalse();
        result.ValidationErrors.Should().Contain("No tasks were generated from the phase brief.");
    }
    ...
    public async Task CreateTaskPlanAsync_WhenTaskMissingTitle_ReturnsInvalidPlan()
    {
        ...
        // Assert
        result.IsValid.Should().BeFalse();
        result.ValidationErrors.Should().Contain("Task missing required field: Title");
    }
    ...
    private static string CreateValidLlmResponse(int taskCount)
    {
        ...
        for (int i = 1; i <= taskCount; i++)
        {
            tasks.Add(new
            {
                ...
                fileScopes = new[]
                {
                    new { relativePath = $"src/file{i}.cs", scopeType = "modify", description = "Update file", mustExist = true }
                },
                verificationSteps = new[]
                {
                    new { verificationType = "compile", description = "Code compiles", expectedOutcome = "Success" }
                },
        ...
        var response = new
        {
            tasks = new[]
            {
                new
                {
                    ...
                    fileScopes = new[] { new { relativePath = "src/file.cs", scopeType = "modify", description = "Update", mustExist = true } },
                    verificationSteps = new[] { new { verificationType = "compile", description = "Compiles", expectedOutcome = "Success" } },
                    acceptanceCriteria = new[] { "Criterion 1" },
                    complexity = "medium",
                    dependencies = Array.Empty<string>()
                }
            }
