...
public class AtomicGitCommitterIntegrationTests : IDisposable
{
    ...
    public async Task CommitAsync_SuccessfulCommitWithScopedFiles_UpdatesEvidence()
    {
        ...
        var runId = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-test001";
    ...
    public async Task CommitAsync_ForbiddenFiles_NotStagedEvenIfModified()
    {
        ...
        await InitializeGitRepoAsync();
        var workspace = CreateTestWorkspace();
        var runId = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-test002";
        ...
        Assert.Contains("tests/Forbidden.cs", statusResult.Stdout); // Still untracked/modified
        Assert.Contains("README.md", statusResult.Stdout); // Still untracked/modified
        Assert.DoesNotContain("src/InScope.cs", statusResult.Stdout); // Staged and committed

        // Verify commit only contains in-scope file
        var logResult = await _gitRunner.ExecuteAsync("log -1 --name-only");
        Assert.Contains("src/InScope.cs", logResult.Stdout);
        Assert.DoesNotContain("tests/Forbidden.cs", logResult.Stdout);
        Assert.DoesNotContain("README.md", logResult.Stdout);
    ...
    public async Task CommitAsync_EmptyIntersection_ProducesNullCommitInEvidence()
    {
        ...
        await InitializeGitRepoAsync();
        var workspace = CreateTestWorkspace();
        var runId = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-test003";
    ...
    public async Task CommitAsync_CommitMessageFormat_IsTSKBased()
    {
        ...
        await InitializeGitRepoAsync();
        var workspace = CreateTestWorkspace();
        var runId = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-test004";
    ...
    public async Task CommitAsync_Rerun_ProducesNewCommitHashAndUpdatedEvidence()
    {
        ...
        // First run
        var runId1 = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-run001";
        ...
        var runId2 = $"RUN-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}-run002";
