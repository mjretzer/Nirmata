...
public sealed class AosValidateWorkspaceRoadmapMissingFromCatalogIndexTests
{
    [Fact]
    public async Task AosValidateWorkspace_FailsDeterministicallyWhenRoadmapReferencesArtifactMissingFromCatalogIndex()
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-validate-workspace-roadmap-missing-from-catalog-index");
        try
        {
            ...
            // Valid roadmap JSON, references an existing artifact, but the id is missing from the catalog index.
            ...
            Assert.Contains(
                "FAIL [spec] .aos/spec/roadmap.json - Roadmap item id 'MS-0001' is not present in catalog index '.aos/spec/milestones/index.json'.",
                stdout,
