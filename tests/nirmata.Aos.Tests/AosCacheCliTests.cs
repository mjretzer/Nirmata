...
public sealed class AosCacheCliTests
{
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-cache-clear");
        try
        {
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-cache-prune");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-cache-prune-days0");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-cache-locked");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("nirmata-aos-cache-validate");
        try
        {
            var aosDllPath = typeof(AosWorkspaceBootstrapper).Assembly.Location;
            Assert.True(File.Exists(aosDllPath), $"aos assembly not found at '{aosDllPath}'.");
