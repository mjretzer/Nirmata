...
public sealed class AosSpecStoreCrudTests
{
    ...
    {
        var repoRoot = CreateTempDirectory("nirmata-aos-spec-store-crud");
        try
        {
    ...
    {
        var repoRoot = CreateTempDirectory("nirmata-aos-spec-store-update-delete");
        try
        {
            var aosRootPath = AosWorkspaceBootstrapper.EnsureInitialized(repoRoot).AosRootPath;
            var store = new AosSpecStore(aosRootPath);
    ...
    {
        var repoRoot = CreateTempDirectory("nirmata-aos-spec-store-task-sub-artifacts");
        try
        {
            var aosRootPath = AosWorkspaceBootstrapper.EnsureInitialized(repoRoot).AosRootPath;
            var store = new AosSpecStore(aosRootPath);
    ...
    {
        var repoRoot = CreateTempDirectory("nirmata-aos-spec-store-project-roadmap");
        try
        {
            var aosRootPath = AosWorkspaceBootstrapper.EnsureInitialized(repoRoot).AosRootPath;
            var store = new AosSpecStore(aosRootPath);
            ...
            var project = new ProjectSpecDocument(
                SchemaVersion: 1,
                Project: new ProjectSpec(Name: "nirmata", Description: "Test")
            );

            store.WriteProjectOverwrite(project);
            Assert.Equal("nirmata", store.ReadProject().Project.Name);
