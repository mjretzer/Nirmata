...
public sealed class AosPathRouterTests
{
    ...
    {
        var aosRootPath = Path.Combine(Path.GetTempPath(), "nirmata-aos-router-tests", ".aos");
    ...
    public void WorkspaceLockContractPath_IsCanonical_AndResolvesDeterministically()
    {
        ...
        var aosRootPath = Path.Combine(Path.GetTempPath(), "nirmata-aos-router-tests", Guid.NewGuid().ToString("N"), ".aos");
    ...
    [Theory]
    [InlineData(".aos/spec/../../secret.txt")]
    [InlineData(".aos/cache/../../../etc/passwd")]
    [InlineData("..")]
    [InlineData("./..")]
    [InlineData("/absolute/path")]
    [InlineData("C:\\Windows")]
    public void ValidateSubpath_RejectsTraversalAndUnsafePaths(string unsafePath)
    {
        Assert.Throws<ArgumentException>(() => AosPathRouter.ValidateSubpath(unsafePath));
    }

    [Theory]
    [InlineData(".aos/spec/milestones/MS-0001/milestone.json")]
    [InlineData(".aos/cache/entry-1")]
    [InlineData(".aos/state/events.ndjson")]
    public void ValidateSubpath_AcceptsSafePaths(string safePath)
    {
        // Should not throw
        AosPathRouter.ValidateSubpath(safePath);
    }

    [Fact]
    public void ToAosRootPath_PreventsEscape()
    {
        var root = "C:\\Workspace";
        
        // Valid
        var valid = AosPathRouter.ToAosRootPath(root, ".aos/spec/milestones/MS-0001/milestone.json");
        Assert.StartsWith(root, valid);

        // Invalid
        Assert.Throws<ArgumentException>(() => AosPathRouter.ToAosRootPath(root, ".aos/spec/../../secret.txt"));
    }
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "nirmata-aos-engine-snapshots", Guid.NewGuid().ToString("N"));
