...
using nirmata.Agents.Execution.ControlPlane.Tools.Mcp;
...
public class ToolRegistryIntegrationTests
{
    ...
    public async Task EndToEnd_RegisterInternalTool_ResolveAndInvoke_ReturnsSuccessResult()
    {
        ...
        var resolvedTool = registry.Resolve("nirmata:aos:tool:test:echo");
    ...
    public async Task EndToEnd_RegisterMultipleTools_ResolveByName_Invoke_ReturnsCorrectResult()
    {
        ...
        registry.Register(
            new ToolDescriptor
            {
                Id = "nirmata:aos:tool:test:toolA",
                Name = "ToolA",
        ...
        registry.Register(
            new ToolDescriptor
            {
                Id = "nirmata:aos:tool:test:toolB",
                Name = "ToolB",
    ...
    public async Task EndToEnd_FullToolLifecycle_RegisterListResolveInvoke()
    {
        ...
        var descriptor = new ToolDescriptor
        {
            Id = "nirmata:aos:tool:calculator:add",
            Name = "CalculatorAdd",
        ...
        Assert.Single(tools);
        Assert.Equal("nirmata:aos:tool:calculator:add", tools[0].Id);
        Assert.Equal("CalculatorAdd", tools[0].Name);

        // Verify descriptor can be resolved
        var resolvedDescriptor = registry.ResolveDescriptor("nirmata:aos:tool:calculator:add");
        Assert.NotNull(resolvedDescriptor);
        Assert.Equal(2, resolvedDescriptor.Parameters.Count);

        // Invoke the tool through registry
        var resolvedTool = registry.Resolve("nirmata:aos:tool:calculator:add");
        Assert.NotNull(resolvedTool);
    ...
    public async Task EndToEnd_MixedInternalAndMcpTools_BothWorkThroughRegistry()
    {
        ...
        registry.Register(
            new ToolDescriptor
            {
                Id = "nirmata:aos:tool:internal:logger",
                Name = "Logger",
        ...
        Assert.Equal("nirmata:aos:tool:internal:logger", tools[0].Id);
        Assert.Equal("mcp:search:query", tools[1].Id);

        // Invoke internal tool
        var internalTool = registry.Resolve("nirmata:aos:tool:internal:logger");
