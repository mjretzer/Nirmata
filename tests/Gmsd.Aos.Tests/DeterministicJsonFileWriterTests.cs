...
public sealed class DeterministicJsonFileWriterTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "gmsd-deterministic-json-no-churn", Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "gmsd-deterministic-json-atomic-failure", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var path = Path.Combine(root, "artifact.json");
