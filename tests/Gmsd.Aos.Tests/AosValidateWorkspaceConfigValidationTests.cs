...
public sealed class AosValidateWorkspaceConfigValidationTests
{
    ...
    {
        var tempWorkspaceRoot = CreateTempDirectory("gmsd-aos-validate-workspace-config-invalid");
        try
        {
            ...
            Assert.Contains(
                "FAIL [config] .aos/config/config.json - (gmsd:aos:schema:config:v1 @ /secrets/k)",
                stdout,
