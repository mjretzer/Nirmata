...
namespace nirmata.Agents.Tests.Execution.Planning.PhasePlanner.Assumptions;
...
public class PhaseAssumptionListerTests
{
    ...
    [Fact]
    public async Task ExtractAssumptionsAsync_WithNullRunId_ThrowsArgumentNullException()
    {
        // Arrange
        var brief = CreateValidPhaseBrief();
        var taskPlan = CreateValidTaskPlan();
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.ExtractAssumptionsAsync(brief, taskPlan, null!));
    }
    ...
    [Fact]
    public async Task GenerateAssumptionsDocumentAsync_WithNullRunId_ThrowsArgumentNullException()
    {
        // Arrange
        var assumptions = new List<PhaseAssumption>();

        // Act & Assert
        await Assert.ThrowsAsync<ArgumentNullException>(() => _sut.GenerateAssumptionsDocumentAsync(assumptions, null!));
    }
        ...
        return new TaskPlan
        {
            ...
            Tasks = new List<TaskSpecification>
            {
                new()
                {
                    ...
                    {
                        new() { Path = "test.cs", ScopeType = "modify" }
                    },
