...
using ConfirmationStateStore = nirmata.Agents.Execution.Preflight.ConfirmationStateStore;
...
public class DestructiveOperationConfirmationE2ETests : IDisposable
{
    ...
    public DestructiveOperationConfirmationE2ETests()
    {
        ...
        _tempWorkspace = Path.Combine(Path.GetTempPath(), $"nirmata-e2e-test-{Guid.NewGuid():N}");
    ...
    public void E2E_DestructiveGitCommit_RequiresConfirmation()
    {
        ...
        // Assert.Equal(nirmata.Agents.Execution.ControlPlane.RiskLevel.WriteDestructiveGit, result.Request!.RiskLevel);
    }
    ...
    public void E2E_ConfirmationState_PersistedToDisk()
    {
        ...
        var action = new nirmata.Agents.Execution.ControlPlane.ProposedAction
        {
            Phase = "Executor",
        ...
        // Act
        _stateStore.SavePendingConfirmation(request, action, nirmata.Agents.Execution.ControlPlane.RiskLevel.WriteSafe);
    ...
    public void E2E_RejectConfirmation_StateUpdated()
    {
        ...
        _stateStore.SavePendingConfirmation(request, action, nirmata.Agents.Execution.ControlPlane.RiskLevel.WriteDestructive);
