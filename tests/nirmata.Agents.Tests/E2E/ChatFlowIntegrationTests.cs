...
public class ChatFlowIntegrationTests
{
    ...
    public async Task ChatFlow_WithWorkspaceState_ResponseIsSuccessful()
    {
        ...
        var fakeCommandRegistry = new FakeCommandRegistry();
        fakeCommandRegistry.AddCommand(new nirmata.Agents.Execution.Preflight.CommandRegistration
        {
            Name = "status",
            Group = "query",
            SideEffect = nirmata.Agents.Execution.Preflight.SideEffect.ReadOnly,
