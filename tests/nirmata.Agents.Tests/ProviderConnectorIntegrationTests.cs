...
public class ProviderConnectorIntegrationTests
{
    ...
    public async Task OpenAiConnector_WithMockedHttpClient_ReturnsChatCompletion()
    {
        ...
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["nirmataAgents:SemanticKernel:OpenAi:BaseUrl"] = "http://localhost:9999"
            })
            .Build();
    ...
    public async Task OpenAiConnector_WithMockedHttpClient_HandlesErrorResponse()
    {
        ...
        var configuration = new ConfigurationBuilder()
            ...
            {
                ["nirmataAgents:SemanticKernel:Provider"] = "OpenAi",
                ["nirmataAgents:SemanticKernel:OpenAi:ApiKey"] = "invalid-key",
                ["nirmataAgents:SemanticKernel:OpenAi:ModelId"] = "gpt-4o"
            })
            .Build();
        ...
        // Act & Assert
        await Assert.ThrowsAsync<Microsoft.SemanticKernel.HttpOperationException>(async () =>
    ...
    public void AzureOpenAiConnector_WithCustomApiVersion_RegistersCorrectly()
    {
        ...
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["nirmataAgents:SemanticKernel:AzureOpenAi:ApiKey"] = "test-azure-key",
                ["nirmataAgents:SemanticKernel:AzureOpenAi:DeploymentName"] = "gpt-4-deployment",
                ["nirmataAgents:SemanticKernel:AzureOpenAi:ApiVersion"] = customApiVersion
            })
            .Build();
    ...
    public async Task ToolAutoInvocation_WithSingleTool_InvokesToolAutomatically()
    {
        ...
        if (descriptorProperty is null)
        {
            ...
            var plugin = nirmata.Agents.Execution.ControlPlane.Llm.Tools.KernelPluginFactory.CreateFromTools(tools);
    ...
    public void ToolRegistry_Integration_WithSemanticKernel()
    {
        ...
        // Act
        var plugin = nirmata.Agents.Execution.ControlPlane.Llm.Tools.KernelPluginFactory.CreateFromRegistry(registry);
...
public class MockStreamingHttpMessageHandler : HttpMessageHandler
{
    ...
    {
        var fullContent = string.Concat(_chunks);
        var content = new StringContent(fullContent);
